...
$pageTitle = "IMPORT CHARACTER EXPRESSIONS";
...
if (isset($_REQUEST['api_action'])) {
    ...
    try {
        ...
        $expr_from = intval($req['expression_from'] ?? 1);
        $expr_to   = intval($req['expression_to'] ?? 999);
        ...
        if ($action === 'preview' || $action === 'import') {
            ...
            if (!empty($exclusions)) {
                ...
                $excludeSql = "AND CONCAT(v.character_id, '_', v.expression_id, '_', v.angle_id, '_', v.perspective_id) NOT IN ($exclList)";
            }
            
            $anglePl = implode(',', array_fill(0, count($angles), '?'));
            $perspPl = implode(',', array_fill(0, count($perspectives), '?'));
            $params = array_merge([$char_from, $char_to, $expr_from, $expr_to], $angles, $perspectives);
            
            $baseWhere = "WHERE v.character_id BETWEEN ? AND ? 
                          AND v.expression_id BETWEEN ? AND ? 
                          AND v.angle_id IN ($anglePl)
                          AND v.perspective_id IN ($perspPl) $excludeSql";
                          
            $notExistsClause = "AND NOT EXISTS (SELECT 1 FROM character_expressions ce WHERE ce.character_id = v.character_id AND ce.expression_id = v.expression_id AND ce.angle_id = v.angle_id AND ce.perspective_id = v.perspective_id)";
            $existsClause = "AND EXISTS (SELECT 1 FROM character_expressions ce WHERE ce.character_id = v.character_id AND ce.expression_id = v.expression_id AND ce.angle_id = v.angle_id AND ce.perspective_id = v.perspective_id)";
            ...
            if ($neutralBg) {
                $promptSql = "CONCAT('((character facing left)), ((looking left)), ((neutral greenscreen background - Pantone: 354 C, Hex: #00FB00)), ', $promptSql)";
            }
            ...
            if ($action === 'import') {
                ...
                $insertCols = "name, `order`, description, character_id, expression_id, angle_id, perspective_id, regenerate_images";
                $selectCols = "CONCAT(v.character_name, ' - ', v.expression_name, ' - ', v.angle_name, ' - ', v.perspective_name), 0, $promptSql, v.character_id, v.expression_id, v.angle_id, v.perspective_id, 1";
                ...
                if ($force) {
                    ...
                    if ($imgId > 0) $updateSql .= ", img2img_frame_id = VALUES(img2img_frame_id), img2img = VALUES(img2img)";
                    
                    $sql = "INSERT INTO character_expressions ($insertCols) SELECT $selectCols FROM v_character_expression_angle_combinations v $baseWhere ON DUPLICATE KEY UPDATE $updateSql";
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute($params);
                    $updated = $stmt->rowCount(); 
                } else {
                    $sql = "INSERT INTO character_expressions ($insertCols) SELECT $selectCols FROM v_character_expression_angle_combinations v $baseWhere $notExistsClause";
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute($params);
                ...
                echo json_encode(['status'=>'success', 'message'=>"Success! Processed $updated existing items and inserted $inserted new ones. Regeneration triggered."]);
                exit;
            } else {
                // PREVIEW
                ...
                $stmt = $pdo->prepare("SELECT COUNT(*) FROM v_character_expression_angle_combinations v $baseWhere"); $stmt->execute($params); $totalMatches = (int)$stmt->fetchColumn();
                $stmt = $pdo->prepare("SELECT COUNT(*) FROM v_character_expression_angle_combinations v $baseWhere $notExistsClause"); $stmt->execute($params); $insertableMatches = (int)$stmt->fetchColumn();
                $updateMatches = $totalMatches - $insertableMatches;
                
                $newRows =[];
                if ($insertableMatches > 0) {
                    $sql = "SELECT v.character_id, v.expression_id, v.angle_id, v.perspective_id, v.character_name, v.expression_name, v.angle_name, v.perspective_name, $promptSql as description 
                            FROM v_character_expression_angle_combinations v $baseWhere $notExistsClause 
                            ORDER BY v.character_id, v.expression_id LIMIT $limit OFFSET $off_ins";
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute($params);
                ...
                $extRows =[];
                if ($updateMatches > 0) {
                    $sql = "SELECT v.character_id, v.expression_id, v.angle_id, v.perspective_id, v.character_name, v.expression_name, v.angle_name, v.perspective_name,
                            (SELECT f.filename FROM character_expressions ce 
                             JOIN frames_2_character_expressions m ON m.to_id = ce.id 
                             JOIN frames f ON f.id = m.from_id 
                             WHERE ce.character_id = v.character_id AND ce.expression_id = v.expression_id AND ce.angle_id = v.angle_id AND ce.perspective_id = v.perspective_id 
                             ORDER BY m.from_id DESC LIMIT 1) as mapped_thumb
                            FROM v_character_expression_angle_combinations v $baseWhere $existsClause 
                            ORDER BY v.character_id, v.expression_id LIMIT $limit OFFSET $off_upd";
                    $stmt = $pdo->prepare($sql);
                    $stmt->execute($params);
                ...
                echo json_encode([
                    'status'=>'success', 'total'=>$totalMatches, 'new_count'=>$insertableMatches, 'ext_count'=>$updateMatches,
                    'new_rows'=>$newRows, 'ext_rows'=>$extRows, 'page_insert'=>$page_insert, 'page_update'=>$page_update,
                    'max_p_ins' => ceil($insertableMatches/$limit), 'max_p_upd' => ceil($updateMatches/$limit)
                ]);
                exit;
            }
        }
        ...
        // ----------------------------------------------------
        // MAPPING: GET EXPRESSIONS (Tab 2 Logic)
        // ----------------------------------------------------
        if ($action === 'get_exprs_for_mapping') {
            if (empty($angles) || empty($perspectives)) { echo json_encode(['status'=>'success', 'data'=>[]]); exit; }
            
            $anglePl = implode(',', array_fill(0, count($angles), '?'));
            $perspPl = implode(',', array_fill(0, count($perspectives), '?'));
            $params = array_merge([$char_from, $char_to, $expr_from, $expr_to], $angles, $perspectives);
            
            $sql = "SELECT ce.id as expression_record_id, c.name as character_name, e.name as expression_name, ca.name as angle_name, cpe.name as perspective_name,
                    (SELECT f.filename FROM frames_2_character_expressions m JOIN frames f ON f.id=m.from_id WHERE m.to_id = ce.id ORDER BY m.from_id DESC LIMIT 1) as mapped_thumb
                    FROM character_expressions ce
                    ...
                    JOIN camera_perspectives cpe ON ce.perspective_id = cpe.id
                    WHERE ce.character_id BETWEEN ? AND ? 
                      AND ce.expression_id BETWEEN ? AND ? 
                      AND ce.angle_id IN ($anglePl) 
                      AND ce.perspective_id IN ($perspPl)
                    ORDER BY ce.character_id, ce.expression_id, ce.perspective_id, ce.angle_id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
        ...
        // ----------------------------------------------------
        // MAPPING: ASSIGN FRAME TO EXPRESSION
        // ----------------------------------------------------
        if ($action === 'map_frame') {
            $frameId = intval($req['frame_id']);
            $exprId = intval($req['expression_id']);
            
            // Check if already mapped exactly
            $stmt = $pdo->prepare("SELECT 1 FROM frames_2_character_expressions WHERE from_id = ? AND to_id = ?");
            $stmt->execute([$frameId, $exprId]);
            if ($stmt->fetchColumn()) {
                // Toggle off
                $pdo->prepare("DELETE FROM frames_2_character_expressions WHERE from_id = ? AND to_id = ?")->execute([$frameId, $exprId]);
                $mapped = false;
            } else {
                // Insert mapped
                $pdo->prepare("INSERT IGNORE INTO frames_2_character_expressions (from_id, to_id) VALUES (?, ?)")->execute([$frameId, $exprId]);
                $mapped = true;
            }
            
            // Re-fetch the best thumbnail for the UI
            $thumb = $pdo->prepare("SELECT f.filename FROM frames_2_character_expressions m JOIN frames f ON f.id=m.from_id WHERE m.to_id = ? ORDER BY m.from_id DESC LIMIT 1");
            $thumb->execute([$exprId]);
...
} catch (Exception $e) { die("Error fetching camera data: " . $e->getMessage()); }
...
<style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .eh-title { font-size: 1rem; font-weight: 700; letter-spacing: 1px; color: var(--purple); margin: 0; text-transform: uppercase; }
    ...
    .eh-tab.active { color: var(--purple); border-bottom-color: var(--purple); }
    ...
    .chip-label input:checked + .chip-span.persp { background: rgba(139,92,246,0.15); border-color: rgba(139,92,246,0.5); color: var(--purple); }
    ...
    .action-btn.primary { border-color: var(--purple); color: #fff; background: rgba(139,92,246,0.2); }
    .action-btn.primary:hover { background: rgba(139,92,246,0.4); box-shadow: 0 0 15px rgba(139,92,246,0.3); }
    ...
    .b-expr { background: rgba(245,158,11,0.15); border-color: rgba(245,158,11,0.3); color: var(--amber); cursor: pointer; }
    .b-expr:hover { border-color: var(--amber); }
    .b-expr.excluded { background: rgba(255,255,255,0.05) !important; border-color: var(--border) !important; color: var(--text-muted) !important; text-decoration: line-through; opacity: 0.5; }
    
    .b-persp { background: rgba(139,92,246,0.15); border-color: rgba(139,92,246,0.3); color: var(--purple); }
    ...
    .map-expr-item { padding: 12px 15px; border-bottom: 1px solid var(--border); cursor: pointer; display: flex; justify-content: space-between; align-items: center; transition: 0.2s; }
    .map-expr-item:hover { background: rgba(255,255,255,0.03); }
    .map-expr-item.active { background: rgba(139,92,246,0.15); border-left: 4px solid var(--purple); padding-left: 11px; }
    .map-expr-info { font-size: 0.75rem; line-height: 1.4; color: var(--text-muted); }
    .map-expr-info strong { color: var(--text); font-size: 0.8rem; display: block; margin-bottom: 2px; }
...
<div class="eh-layout">
    ...
    <div class="eh-header">
        <div class="eh-header-left">
            ...
            <h1 class="eh-title">IMPORT CHARACTER EXPRESSIONS</h1>
        </div>
    ...
    <form id="appForm" class="eh-body" onsubmit="return false;">
        ...
        <div class="eh-sidebar" id="appSidebar">
            <div class="sidebar-header-mobile">
                <span style="font-weight: bold; color: var(--purple); font-size: 0.85rem; letter-spacing: 1px;">⚙️ FILTERS</span>
            ...
            <div class="sidebar-section">
                <div class="section-title">
                    ...
                    <span><a href="#" onclick="toggleChecks('perspectives[]', true); triggerLoad(); return false;">All</a> | <a href="#" onclick="toggleChecks('perspectives[]', false); triggerLoad(); return false;">None</a></span>
                </div>
            ...
            <div class="sidebar-section">
                <div class="section-title">
                    2. Camera Angles
                    <span><a href="#" onclick="toggleChecks('angles[]', true); triggerLoad(); return false;">All</a> | <a href="#" onclick="toggleChecks('angles[]', false); triggerLoad(); return false;">None</a></span>
                </div>
                <div class="chips-container">
            ...
            <div class="sidebar-section">
                ...
                <div class="form-row-compact">
                    <div class="form-col"><label>Expr From</label><input type="number" name="expression_from" class="dark-input" value="1" oninput="debounceLoad()"></div>
                    <div class="form-col"><label>Expr To</label><input type="number" name="expression_to" class="dark-input" value="999" oninput="debounceLoad()"></div>
                </div>
            </div>
        ...
        <div class="eh-main pswp-gallery">
            ...
            <div id="view-import" class="tab-view" style="display:flex;">
                ...
                <div class="eh-grid-area" id="importContainers">
                    <!-- Dynamic Tables Go Here -->
                    <div class="empty-state">
                        <span>🔍</span><div>Select Filters on the left to generate a preview.</div>
                    </div>
                </div>
            </div>
            ...
            <div id="view-map" class="tab-view map-layout">
                <div class="map-sidebar-list" id="mapExprsList">
                    <div class="empty-state" style="padding:20px;"><span>📋</span><div style="font-size:0.75rem;">Loading expressions...</div></div>
                </div>
...
<div id="descModal" class="modal-overlay" onclick="closeDescModal(event)">
    <div class="modal-content">
        <div class="modal-header">
            <span>Full Description Prompt</span>
...
let timerLoad = null;
let timerRef = null;
let timerFrames = null;
let activeMapExprId = null;
...
async function initPhotoswipe() {
    ...
    // Dynamically import the Lightbox module
...
async function loadPreview() {
    ...
    try {
        ...
        if (res.total === 0) {
            cont.innerHTML = `<div class="empty-state"><span>🔍</span><div>No combinations found. Adjust filters.</div></div>`;
            return;
        }
        ...
        if (res.new_rows && res.new_rows.length > 0) {
            html += `
            <div class="table-wrapper"><div class="table-header-bar h-success"><div>NEW ENTRIES</div><div>Page ${curPageIns}</div></div>
            <table class="dark-table">
                <thead><tr><th>Character</th><th>Expression</th><th>Shot Info</th><th>Description</th></tr></thead><tbody>`;
            res.new_rows.forEach(r => {
                const key = `${r.character_id}_${r.expression_id}_${r.angle_id}_${r.perspective_id}`;
                ...
                html += `<tr>
                    ...
                    <td><span class="badge-chip b-expr ${exCls}" onclick="toggleExpr(this, '${key}')" title="Click to exclude">${escapeHtml(r.expression_name)}</span></td>
        ...
        if (res.ext_rows && res.ext_rows.length > 0) {
            html += `
            <div class="table-wrapper"><div class="table-header-bar h-warning"><div>EXISTING ENTRIES</div><div>Page ${curPageUpd}</div></div>
            <table class="dark-table">
                <thead><tr><th>Thumb</th><th>Character</th><th>Expression</th><th>Shot Info</th><th>Status</th></tr></thead><tbody>`;
            res.ext_rows.forEach(r => {
                ...
                if(r.mapped_thumb) {
                    thumbHtml = `
                    <a href="${escapeHtml(r.mapped_thumb)}" class="pswp-link" style="display:block;">
                        <img src="${escapeHtml(r.mapped_thumb)}" onload="this.parentElement.setAttribute('data-pswp-width', this.naturalWidth); this.parentElement.setAttribute('data-pswp-height', this.naturalHeight);" style="width:32px; height:32px; object-fit:cover; border-radius:4px; border:1px solid #444; cursor:zoom-in;">
                    </a>`;
                }
            ...
            cont.innerHTML = `
                <div style="display:inline-block; width: 60%; max-width: 150px; border: 1px solid var(--border); border-radius: 6px; overflow: hidden; background: #000; box-shadow: 0 4px 10px rgba(0,0,0,0.3);">
                    <a href="${escapeHtml(res.url)}" data-pswp-width="1024" data-pswp-height="1024" class="pswp-link" style="display: block;">
                        <img src="${escapeHtml(res.url)}" onload="this.parentElement.setAttribute('data-pswp-width', this.naturalWidth); this.parentElement.setAttribute('data-pswp-height', this.naturalHeight);" style="width: 100%; display: block; aspect-ratio: 1/1; object-fit: cover; cursor: zoom-in;" alt="Preview Frame">
                    </a>
                </div>
            `;
            initPhotoswipe();
...
async function loadMappingExprs() {
    const fd = new FormData(document.getElementById('appForm'));
    fd.append('api_action', 'get_exprs_for_mapping');
    const cont = document.getElementById('mapExprsList');
    ...
    try {
        ...
        if(!res.data || res.data.length===0) {
            cont.innerHTML = `<div class="empty-state" style="padding:20px;"><span>📋</span><div style="font-size:0.75rem;">No imported expressions found. Adjust filters.</div></div>`;
            return;
        }
        ...
        res.data.forEach(e => {
            let img = '';
            if(e.mapped_thumb) {
                img = `<img src="${escapeHtml(e.mapped_thumb)}" class="map-thumb" />`;
            }
            html += `
            <div class="map-expr-item" id="me_${e.expression_record_id}" onclick="selectMapExpr(${e.expression_record_id})">
                <div class="map-expr-info">
                    <strong>${escapeHtml(e.character_name)}</strong>
                    <span style="color:var(--amber); font-weight:bold;">${escapeHtml(e.expression_name)}</span><br>
                    ${escapeHtml(e.perspective_name)} | ${escapeHtml(e.angle_name)}
                </div>
                <div class="map-thumb-wrap" id="me_img_${e.expression_record_id}">${img}</div>
            </div>`;
        });
        cont.innerHTML = html;
        activeMapExprId = null; // reset
    } catch(e) {}
}
...
async function loadRunFrames() {
    ...
    try {
        ...
        res.data.forEach(f => {
            html += `
            <div class="grid-frame" onclick="assignFrame(${f.id})">
...
function selectMapExpr(id) {
    document.querySelectorAll('.map-expr-item').forEach(el => el.classList.remove('active'));
    const item = document.getElementById('me_' + id);
    if(item) {
...
async function assignFrame(frameId) {
    if (!activeMapExprId) {
        alert("Please click an Expression in the left list first!");
        return;
    }
    ...
    fd.append('expression_id', activeMapExprId);
    
    try {
        const res = await fetch('?', {method:'POST', body:fd}).then(r=>r.json());
        if (res.status==='success') {
            const imgWrap = document.getElementById('me_img_' + activeMapExprId);
            if (res.thumb) {
                imgWrap.innerHTML = `<a href="${escapeHtml(res.thumb)}" class="pswp-link" style="display:block;width:100%;height:100%;"><img src="${escapeHtml(res.thumb)}" class="map-thumb" onload="this.parentElement.setAttribute('data-pswp-width', this.naturalWidth); this.parentElement.setAttribute('data-pswp-height', this.naturalHeight);" style="cursor:zoom-in;" /></a>`;
            } else {
                imgWrap.innerHTML = ``;
            }
            initPhotoswipe();
            
            // Auto-advance if we successfully mapped it (not toggled off)
            if (res.mapped) {
                const currentItem = document.getElementById('me_' + activeMapExprId);
                const nextItem = currentItem.nextElementSibling;
                if (nextItem && nextItem.classList.contains('map-expr-item')) {
                    const nextId = nextItem.id.replace('me_', '');
                    selectMapExpr(nextId);
                }
            }
        }
    } catch(e){}
