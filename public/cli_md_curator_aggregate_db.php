...
// DB/JSON-capable:
// - Preserves the original processing logic
// - NEW: Accepts --qjobs=X to pull from database queue (forge_jobs)
// - Falls back to original interactive category selection when no config is provided
...
$optsOuter = getopt('', ['cat::', 'limit::', 'config::', 'config-json::', 'qjobs::']);
...
$jobsToProcess = [];
if ($qjobs > 0) {
    $jobsToProcess = $conn->fetchAllAssociative("SELECT * FROM forge_jobs WHERE job_type = 'md_curator_aggregate' AND status = 'pending' ORDER BY priority ASC, id ASC LIMIT " . $qjobs);
    if (empty($jobsToProcess)) {
        echo "No pending md_curator_aggregate jobs found.\n";
        exit(0);
    }
} else {
...
foreach ($jobsToProcess as $jobRow) {
    ...
    try {
        if ($jobId) {
            ...
            if (isset($cfg['cat']) && $targetCategoryId === 0) {
                $targetCategoryId = (int)$cfg['cat'];
            }
            if (isset($cfg['limit'])) {
                $limit = (int)$cfg['limit'];
            }
            $jsonMode = true;
            echo "\n=== RUNNING QUEUE JOB #$jobId (md_curator_aggregate) ===\n";
        } else {
            if (isset($optsOuter['config'])) {
                $jsonMode = true;
                $cfgRaw = file_get_contents($optsOuter['config']);
                $cfg = json_decode($cfgRaw ?: '', true);
                if (!is_array($cfg)) throw new RuntimeException("Error: Invalid JSON config file.");
                $targetCategoryId = array_key_exists('targetCategoryId', $cfg) ? (int)$cfg['targetCategoryId'] : 0;
                if (isset($cfg['cat']) && $targetCategoryId === 0) $targetCategoryId = (int)$cfg['cat'];
                if (isset($cfg['limit'])) $limit = (int)$cfg['limit'];
            } elseif (array_key_exists('config-json', $optsOuter) && is_string($optsOuter['config-json']) && trim($optsOuter['config-json']) !== '') {
                $jsonMode = true;
        ...
        if (empty($docs)) throw new RuntimeException("No docs found or all docs are locked.");
            ...
            try {
                ...
                foreach ($agEntitiesMap as $agNodeType => $entitiesList) {
                    foreach ($entitiesList as $ent) {
                        ...
                        $tmp = ($ent['name'] ?? $ent['title'] ?? $ent['hook'] ?? $ent['scene'] ?? '');
                        $entName = mb_substr(trim(is_array($tmp) ? json_encode($tmp, JSON_UNESCAPED_UNICODE) : (is_string($tmp) && ($j = json_decode($tmp, true)) !== null ? json_encode($j, JSON_UNESCAPED_UNICODE) : $tmp)), 0, 1024, 'UTF-8');
