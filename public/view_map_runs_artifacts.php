<?php
// public/view_map_runs_artifacts.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'regenerate_run') {
    ...
    if ($map_run_id) {
        try {
            // Find all artifact entity IDs linked to frames in this run
            $stmt = $pdo->prepare("SELECT DISTINCT entity_id FROM frames WHERE map_run_id = :mid AND entity_type = 'artifacts'");
            ...
            if (!empty($ids)) {
                ...
                $upStmt = $pdo->prepare("UPDATE artifacts SET regenerate_images = 1 WHERE id IN ($inQuery)");
                $upStmt->execute($ids);
            }
...
// Base Condition
$whereClause = "WHERE mr.entity_type = 'artifacts'";
$params = [];
...
$entities_with_menu = ['characters', 'character_poses', 'animas', 'locations', 'backgrounds', 'artifacts', 'vehicles', 'scene_parts', 'controlnet_maps', 'spawns', 'artifacts', 'sketches', 'prompt_matrix_blueprints', 'composites'];
...
function getFramesForRun($pdo, $map_run_id) {
    ...
    SELECT 
        ...
        f.entity_type,
        f.entity_id,
        g.name as artifact_name,
        ie.tool as edit_tool,
        ie.note as edit_note,
        f.img2img_frame_id
        
    FROM frames f
    LEFT JOIN artifacts g ON g.id = f.entity_id AND f.entity_type = 'artifacts'
...
<head>
    ...
    <title>artifacts Map Runs</title>
...
<body>
    ...
    <div class="header-compact">
        <a href="sql_crud_artifacts.php" class="entity-icon-link" title="artifacts CRUD"><?php echo $iconChar; ?></a>
        <a href="gallery_artifacts_nu.php" class="entity-icon-link" title="artifacts Gallery">▦</a>
        <h2 style="margin:0; font-size:1.2rem; color:var(--text);">artifacts Map Runs</h2>
        <?php if ($schedulerId): ?>
    ...
    <div class="container">
                                ...
                                <div class="chain-card" 
                                    ...
                                    <div class="chain-card-body">
                                        ...
                                        <div class="chain-card-meta">
                                            ...
                                            <?php if (!empty($frame['artifact_name'])): ?>
                                                 <span class="badge badge-blue"><?= htmlspecialchars($frame['artifact_name']) ?></span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </div>
        ...
        <?php else: ?>
            <div class="empty-state">
                <p>No map runs found for artifacts.</p>
                <?php if($search): ?>
    ...
    <div id="meta-modal" class="modal-overlay">
        <div class="modal-content">
            ...
            <h3 class="modal-title">artifact Meta Information</h3>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        $(document).on('click', '.regen-run-btn', function(e) {
            ...
            if (!confirm("Are you sure you want to mark all artifacts in this map run (Run #" + runId + ") for regeneration?")) {
                return;
            }
            ...
            $.post('', { // Post to current file
                ...
                if (response.success) {
                    Toast.show('Success: ' + response.count + ' artifact(s) marked for regeneration.', 'success');
