<?php
// public/view_collections_admin.php
require_once __DIR__ . '/bootstrap.php';
$spw = \App\Core\SpwBase::getInstance();
$pdo = $spw->getPDO();
$pageTitle = "Chroma Collections Admin";
ob_start();

try {
    $stmt = $pdo->query("SELECT * FROM chroma_collections ORDER BY name ASC");
...
<style>
/* Same styles as original, with minor adjustments */
.admin-wrap { max-width: 900px; margin: 0 auto; padding: 18px; color: var(--text); }
...
.item-meta { font-size: 0.85rem; color: var(--text-muted); margin-top: 4px; display: flex; gap: 12px; flex-wrap: wrap; }
...
.modal-card { width: 100%; max-width: 550px; background: var(--card); border-radius: 10px; box-shadow: var(--card-elevation); }
...
<div class="admin-wrap">
    <div class="admin-head">
        <h2>Manage Chroma Collections</h2>
        <div>
            <a class="btn btn-sm btn-outline-secondary" href="dashboard.php">&larr; Back to Dashboard</a>
            <button class="btn btn-sm btn-primary" onclick="openCreateModal()">+ New Collection</button>
        </div>
    </div>
        ...
        <?php foreach ($rows as $row): ?>
            <div class="list-item" data-id="<?= (int)$row['id'] ?>">
                <div class="item-info">
                    ...
                    <div class="item-meta">
                        <span>Type: <?= htmlspecialchars($row['type']) ?></span>
                        <span>Dim: <?= (int)$row['dimension'] ?></span>
                        <?php if (!empty($row['description'])): ?>
                            <span>Description: <?= htmlspecialchars(substr($row['description'], 0, 50)) ?><?= strlen($row['description']) > 50 ? '…' : '' ?></span>
                        <?php endif; ?>
                    </div>
                </div>
                <div class="actions">
...
<div id="formModal" class="modal-overlay">
    ...
    <div class="form-row">
        ...
        <select id="type" name="type" class="form-select" required onchange="setDimensionFromType()">
            <option value="">-- Select --</option>
            <option value="text" data-dim="384">text - MiniLM (384-dim)</option>
            <option value="text" data-dim="512">text - CLIP (512-dim)</option>
            <option value="image" data-dim="512">image - CLIP (512-dim)</option>
        </select>
    </div>
    <div class="form-row">
        <label class="form-label">Dimension *</label>
        <input type="number" id="dimension" name="dimension" class="form-input" min="1" step="1" required>
    </div>
    <div class="form-row">
        ...
        <div class="modal-footer">
            ...
            <button class="btn btn-sm btn-primary" onclick="saveItem()">Save Collection</button>
        </div>
    </div>
</div>
...
<script>
const API_URL = '/collections_api.php';
const ENTITY_NAME = 'Collection';
...
// Auto‑fill dimension when a preset type/model is selected
function setDimensionFromType() {
...
function openCreateModal() {
    document.getElementById('modalTitle').textContent = `Create ${ENTITY_NAME}`;
    document.getElementById('itemId').value = '';
    document.getElementById('name').value = '';
    document.getElementById('type').value = 'text';          // selects first text option (MiniLM)
    setDimensionFromType();                                   // fills 384
...
async function openEditModal(id) {
    ...
    if (result.status === 'ok' && result.data) {
        document.getElementById('modalTitle').textContent = `Edit ${ENTITY_NAME}`;
        document.getElementById('itemId').value = result.data.id;
        document.getElementById('name').value = result.data.name;
        document.getElementById('type').value = result.data.type;
        document.getElementById('dimension').value = result.data.dimension;
        document.getElementById('description').value = result.data.description || '';
        document.getElementById('formModal').classList.add('active');
    } else {
        showToast(`Failed to load ${ENTITY_NAME.toLowerCase()}`, 'error');
    }
}
...
async function saveItem() {
    ...
    if (!data.name || !data.type || !data.dimension) {
        showToast('Name, Type and Dimension are required', 'error');
        return;
    }
    const result = await apiCall('save', data);
    if (result.status === 'ok') {
        showToast(`${ENTITY_NAME} saved`, 'success');
        location.reload();
    } else {
        showToast(result.message || `Failed to save ${ENTITY_NAME.toLowerCase()}`, 'error');
    }
}
...
async function deleteItem(id) {
    ...
    if (result.status === 'ok') {
        showToast(`${ENTITY_NAME} deleted`, 'success');
        location.reload();
    } else {
        showToast(result.message || `Failed to delete ${ENTITY_NAME.toLowerCase()}`, 'error');
    }
}
</script>
<?php
