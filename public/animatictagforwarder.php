<?php
// public/animatictagforwarder.php
// Animatic Tag Forwarder -- Mass propagates tags from Source Frames to Animatics and Videos
// ----------------------------------------------------
...
if (isset($_REQUEST['api_action'])) {
    ...
    try {
        if ($action === 'get_animatic_ids') {
            $from = (int)$_POST['from_id'];
            $to = (int)$_POST['to_id'];
            
            // Only fetch Animatics that actually have a source frame linked
            $sql = "SELECT id FROM animatics WHERE id >= ? AND id <= ? AND img2img_frame_id IS NOT NULL ORDER BY id ASC";
        ...
        if ($action === 'process_batch') {
            $animaticIds = json_decode($_POST['animatic_ids'], true);
            if (empty($animaticIds)) {
                echo json_encode(['status' => 'success', 'inserted_animatics' => 0, 'inserted_videos' => 0, 'log' => 'Empty batch']);
                exit;
            }

            $inClause = implode(',', array_fill(0, count($animaticIds), '?'));
            
            // 1. Forward tags to tags_2_animatics
            $sqlAnimatics = "
                INSERT IGNORE INTO tags_2_animatics (from_id, to_id)
                SELECT t2f.from_id AS tag_id, a.id AS animatic_id
                FROM animatics a
                JOIN tags_2_frames t2f ON t2f.to_id = a.img2img_frame_id
                WHERE a.id IN ($inClause)
            ";
            $stmtA = $pdo->prepare($sqlAnimatics);
            $stmtA->execute($animaticIds);
            $insertedAnimatics = $stmtA->rowCount();

            // 2. Forward tags to tags_2_videos
            $sqlVideos = "
                INSERT IGNORE INTO tags_2_videos (from_id, to_id)
                SELECT t2f.from_id AS tag_id, v2a.from_id AS video_id
                FROM animatics a
                JOIN tags_2_frames t2f ON t2f.to_id = a.img2img_frame_id
                JOIN videos_2_animatics v2a ON v2a.to_id = a.id
                WHERE a.id IN ($inClause)
            ";
            $stmtV = $pdo->prepare($sqlVideos);
            $stmtV->execute($animaticIds);
            $insertedVideos = $stmtV->rowCount();
            
            // Generate log
            $minId = min($animaticIds);
            $maxId = max($animaticIds);
            
            $logMsg = "Animatics #$minId to #$maxId: Forwarded frame tags to Animatics (+$insertedAnimatics) and Videos (+$insertedVideos).";

            echo json_encode([
                'status' => 'success', 
                'inserted_animatics' => $insertedAnimatics,
                'inserted_videos' => $insertedVideos,
                'log' => $logMsg
            ]);
            exit;
        }
...
$pageTitle = 'Animatic Tag Forwarder';
ob_start();
?>
<!-- Dependencies -->
...
<div class="eh-layout">
    <div class="eh-header">
        <div class="eh-title"><span>🎬</span> ANIMATIC TAG FORWARDER</div>
    </div>

    <div class="eh-controls">
        <div class="input-group">
            <label>From Animatic ID</label>
            <input type="number" id="fromId" class="run-input" placeholder="e.g. 1000">
        </div>
        <div class="input-group">
            <label>To Animatic ID</label>
    ...
    <div class="eh-console-area">
        ...
        <div class="console-scroll" id="consoleOut">
            <div class="log-line"><span class="log-time">[SYSTEM]</span> Ready. Enter an Animatic ID range to forward source-frame tags downstream.</div>
        </div>
    </div>

    <div class="stats-bar">
        <div class="stat-item">
            <div class="stat-val" id="statAnimatics">0</div>
            <div class="stat-label">Animatics Processed</div>
        </div>
        <div class="stat-item">
            <div class="stat-val" id="statTagsAnimatics" style="color: var(--amber);">0</div>
            <div class="stat-label">Tags → Animatics</div>
        </div>
        <div class="stat-item">
            <div class="stat-val" id="statTagsVideos" style="color: var(--green);">0</div>
            <div class="stat-label">Tags → Videos</div>
        </div>
    </div>
</div>

<script>
let totalAnimatics = 0;
let totalTagsAnimatics = 0;
let totalTagsVideos = 0;
...
function updateStats() {
    document.getElementById('statAnimatics').innerText = totalAnimatics;
    document.getElementById('statTagsAnimatics').innerText = totalTagsAnimatics;
    document.getElementById('statTagsVideos').innerText = totalTagsVideos;
}
...
async function startForwarding() {
    ...
    totalAnimatics = 0;
    totalTagsAnimatics = 0;
    totalTagsVideos = 0;
    updateStats();

    log(`Fetching animatics in range ${fromId} - ${toId}...`);
    ...
    try {
        ...
        const r = await fetch('?api_action=get_animatic_ids', {method: 'POST', body: fd});
        const res = await r.json();

        if (res.status !== 'success') throw new Error(res.message);

        const animaticIds = res.data;
        if (animaticIds.length === 0) {
            log('No animatics found in that range with a linked source frame.', 'warn');
            resetUI();
            return;
        }

        log(`Found ${animaticIds.length} animatics with source frames. Commencing batch forwarding...`, 'info');

        // Chunking array into batches of 50
        const chunkSize = 50;
        for (let i = 0; i < animaticIds.length; i += chunkSize) {
            const chunk = animaticIds.slice(i, i + chunkSize);
            
            const batchFd = new FormData();
            batchFd.append('animatic_ids', JSON.stringify(chunk));

            const bReq = await fetch('?api_action=process_batch', {method: 'POST', body: batchFd});
            const bRes = await bReq.json();

            if (bRes.status === 'success') {
                log(bRes.log, (bRes.inserted_animatics > 0 || bRes.inserted_videos > 0) ? 'success' : 'info');
                totalAnimatics += chunk.length;
                totalTagsAnimatics += bRes.inserted_animatics;
                totalTagsVideos += bRes.inserted_videos;
                updateStats();
            } else {
