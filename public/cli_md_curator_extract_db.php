<?php
// PART 1: EXTRACTOR (Crawler) - DB/JSON-capable
// - Preserves the original extraction logic
// - NEW: Accepts --qjobs=X to pull from database queue (forge_jobs)
//
// Usage:
//   Interactive: php cli_md_curator_extract_db.php
//   CLI args:    php cli_md_curator_extract_db.php --cat=3 --limit=10 --chars=4000 --model=gpt-4o-mini
//   JSON file:   php cli_md_curator_extract_db.php --config=md_curator_job.json
//   Queue Mode:  php cli_md_curator_extract_db.php --qjobs=3
// ----------------------------------------------------
...
$optsOuter = getopt('', ['cat::', 'limit::', 'chars::', 'model::', 'config::', 'config-json::', 'qjobs::']);
...
$jobsToProcess = [];
if ($qjobs > 0) {
    $jobsToProcess = $conn->fetchAllAssociative("SELECT * FROM forge_jobs WHERE job_type = 'md_curator_extract' AND status = 'pending' ORDER BY priority ASC, id ASC LIMIT " . $qjobs);
    if (empty($jobsToProcess)) {
        echo "No pending md_curator_extract jobs found.\n";
        exit(0);
    }
} else {
...
foreach ($jobsToProcess as $jobRow) {
    ...
    try {
        if ($jobId) {
            ...
            $targetCategoryId  = array_key_exists('targetCategoryId', $cfg) ? (int)$cfg['targetCategoryId'] : 0;
            $limit             = array_key_exists('limit', $cfg) ? (int)$cfg['limit'] : $limit;
            $charLimit         = array_key_exists('charLimit', $cfg) ? (int)$cfg['charLimit'] : $charLimit;
            $overrideModel     = isset($cfg['overrideModel']) && trim((string)$cfg['overrideModel']) !== '' ? (string)$cfg['overrideModel'] : null;
            $jsonMode = true;

            echo "\n=== RUNNING QUEUE JOB #$jobId (md_curator_extract) ===\n";
        } else {
            // process $optsOuter for CLI/JSON modes
            if (isset($optsOuter['config'])) {
                $jsonMode = true;
                $cfg = json_decode(file_get_contents($optsOuter['config']), true);
                if (!is_array($cfg)) throw new RuntimeException("Error: Invalid JSON config file.");
                $targetCategoryId  = array_key_exists('targetCategoryId', $cfg) ? (int)$cfg['targetCategoryId'] : 0;
                $limit             = array_key_exists('limit', $cfg) ? (int)$cfg['limit'] : $limit;
                $charLimit         = array_key_exists('charLimit', $cfg) ? (int)$cfg['charLimit'] : $charLimit;
                $overrideModel     = isset($cfg['overrideModel']) && trim((string)$cfg['overrideModel']) !== '' ? (string)$cfg['overrideModel'] : null;

                if (array_key_exists('cat', $cfg) && $targetCategoryId === 0) $targetCategoryId = (int)$cfg['cat'];
                if (array_key_exists('category_id', $cfg) && $targetCategoryId === 0) $targetCategoryId = (int)$cfg['category_id'];
                if (array_key_exists('targetCategoryId', $cfg)) $targetCategoryId = (int)$cfg['targetCategoryId'];
                if (array_key_exists('limit', $cfg)) $limit = (int)$cfg['limit'];
                if (array_key_exists('chars', $cfg) && !array_key_exists('charLimit', $cfg)) $charLimit = (int)$cfg['chars'];
                if (array_key_exists('overrideModel', $cfg)) $overrideModel = trim((string)$cfg['overrideModel']) !== '' ? (string)$cfg['overrideModel'] : null;
            } elseif (isset($optsOuter['config-json']) && trim((string)$optsOuter['config-json']) !== '') {
                $jsonMode = true;
                $cfg = json_decode($optsOuter['config-json'], true);
                if (!is_array($cfg)) throw new RuntimeException("Error: Invalid JSON passed to --config-json.");
                $targetCategoryId  = array_key_exists('targetCategoryId', $cfg) ? (int)$cfg['targetCategoryId'] : 0;
                $limit             = array_key_exists('limit', $cfg) ? (int)$cfg['limit'] : $limit;
                $charLimit         = array_key_exists('charLimit', $cfg) ? (int)$cfg['charLimit'] : $charLimit;
                $overrideModel     = isset($cfg['overrideModel']) && trim((string)$cfg['overrideModel']) !== '' ? (string)$cfg['overrideModel'] : null;
        ...
        if (!$configLore || !$configShow) {
            throw new RuntimeException("Error: Missing generator configs ($curatorConfigId, $showrunnerConfigId).");
        }
        if ($overrideModel) {
        ...
        if ($targetCategoryId === null) {
            ...
            while ($targetCategoryId === null) {
                ...
                if ($val === 0) {
                    ...
                    $targetCategoryId = $map[$val];
                }
            }
        } else {
            if (!is_int($targetCategoryId)) {
        ...
        if ($targetCategoryId > 0) {
            $whereSql .= " AND d.category_id = :cat";
            $params['cat'] = $targetCategoryId;
        }

        $sql = "SELECT d.id, d.name, d.content, d.description
        ...
        if (empty($docs)) throw new RuntimeException("No documents found.");
                ...
                foreach ($parts as $part) {
                    ...
                    if (strlen($part) > $charLimit) {
                        ...
                        foreach ($subParts as $sp) {
                            if (strlen($subBuffer) + strlen($sp) > $charLimit) {
                                $chunks[] = $subBuffer;
                                $subBuffer = $sp;
                            } else {
                                $subBuffer .= "\n\n" . $sp;
                        ...
                        if (strlen($buffer) + strlen($part) > $charLimit) {
                            $chunks[] = $buffer;
                            $buffer = $part;
                        } else {
                            $buffer .= "\n" . $part;
        ...
        if ($jobId) {
            ...
            echo C_RED . $jobEx->getMessage() . C_RESET . "\n";
            exit(1);
        }
    }
}
