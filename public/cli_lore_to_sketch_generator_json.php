<?php
// public/cli_lore_to_sketch_generator_json.php
// Lore -> Sketch Generator Runner
// - CLI-first
// - Accepts JSON config via --config=file.json or --config-json='...'
// - JSON mode is strictly non-interactive
// - Falls back to interactive prompts only when no JSON config is provided
...
try {
    ...
    if (empty($cfg['doc_id'])) {
        if ($strict) {
            throw new RuntimeException("doc_id is required in JSON mode.");
        }
    ...
    if (empty($cfg['group_key'])) {
        if ($strict) {
            throw new RuntimeException("group_key is required in JSON mode.");
        }
    ...
    if ($strict) {
        if (!parseBool($cfg['confirm'])) {
            throw new RuntimeException("confirm must be true in JSON mode.");
        }
    } else {
