...
BabylonViewerJS.prototype._saveInitialCameraState = function() {
    ...
    try {
        this._initialCameraState = {
            ...
            up: this.camera.upVector ? this.camera.upVector.clone() : new BABYLON.Vector3(0,1,0) // <-- store upVector
        };
...
BabylonViewerJS.prototype.resetCamera = function(animate = false) {
    ...
    // restore upVector immediately (no animation) so orientation matches
    try { this.camera.upVector = (s.up && s.up.clone) ? s.up.clone() : this.camera.upVector; } catch(e){}
};
