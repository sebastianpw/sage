<?php
// public/narseq_filter_forge_api.php
// ─────────────────────────────────────────────────────────────────────────────
...
//   fuzz_id[]            — fuzz_candidates.id (array support)
...
//   frame_id             — direct frame id filter
//   search[]             — full-text search (array support)
...
// Support array of Fuzz IDs
$fuzzIdRaw     = $_GET['fuzz_id'] ?? null;
$fuzzIds       = [];
if (is_array($fuzzIdRaw)) {
    $fuzzIds = array_filter(array_map('intval', $fuzzIdRaw));
} elseif ($fuzzIdRaw) {
    $fuzzIds = [(int)$fuzzIdRaw];
}
...
$frameId       = isset($_GET['frame_id'])        ? (int)$_GET['frame_id']     : null;

// Support array of search terms
$searchRaw     = $_GET['search'] ?? null;
$searchTerms   = [];
if (is_array($searchRaw)) {
    $searchTerms = array_filter(array_map('trim', $searchRaw));
} elseif ($searchRaw) {
    $searchTerms = [trim($searchRaw)];
}
...
if ($action === 'list_entities') {
    _listEntities($pdo, $entityType, $fuzzIds, $docId, $docEntityName, $docEntityNames,
                  $kgNodeId, $seqId, $storyboardId, $vectorText, $mapRunId, $entityId, $frameId, $searchTerms,
                  $sort, $page, $perPage, $filterMode);
    exit;
}

// ── LIST FRAMES (default) ─────────────────────────────────────────────
_listFrames($pdo, $entityType, $fuzzIds, $docId, $docEntityName, $docEntityNames,
             $kgNodeId, $seqId, $storyboardId, $vectorText, $mapRunId, $entityId, $frameId,
             $searchTerms, $sort, $page, $perPage, $inclMembership, $filterMode);
exit;
...
function computeForgeIntersection(
    ...
    // ── 1b. Direct frame_id ───────────────────────────────────────────
    if ($frameId) {
        $stmt = $pdo->prepare("SELECT entity_id FROM frames WHERE id = ? AND entity_type = ? AND entity_id IS NOT NULL");
        $stmt->execute([$frameId, $entityType]);
    ...
    if (!empty($fuzzIds)) {
        if ($filterMode === 'intersection') {
            foreach ($fuzzIds as $fid) {
                $stmt = $pdo->prepare(
                    "SELECT DISTINCT source_row_id FROM fuzz_mentions
                     WHERE candidate_id = ? AND source_table = ? AND source_row_id IS NOT NULL"
                );
                $stmt->execute([$fid, $entityType]);
                $sets[] = $stmt->fetchAll(PDO::FETCH_COLUMN);
            }
        } else {
            $ph = implode(',', array_fill(0, count($fuzzIds), '?'));
            $stmt = $pdo->prepare(
                "SELECT DISTINCT source_row_id FROM fuzz_mentions
                 WHERE candidate_id IN ($ph) AND source_table = ? AND source_row_id IS NOT NULL"
            );
            $stmt->execute(array_merge($fuzzIds, [$entityType]));
            $sets[] = $stmt->fetchAll(PDO::FETCH_COLUMN);
        }
    }
...
function _listFrames(
    ...
    $hasEntityFilters = !empty($fuzzIds) || $docId || $kgNodeId || $seqId || $vectorText || $entityId;
    $hasFrameFilters  = $storyboardId || $mapRunId || $frameId;

    $intersectIds = null;
    if ($hasEntityFilters) {
        // Calculate intersection purely for entity constraints.
        // We pass null for storyboard, map_run, and frame to avoid dropping standalone frames.
        $intersectIds = computeForgeIntersection(
            $pdo, $entityType, $fuzzIds, $docId, $docEntityName, $docEntityNames,
            $kgNodeId, $seqId, null, $vectorText, null, $entityId, null, $filterMode
        );
    }

    $joinSql = "LEFT JOIN $entityTable e ON e.id = f.entity_id";
    
    // ── Specifically enforce Storyboard frames ONLY to prevent bleeding ──
    if ($storyboardId) {
        $joinSql .= " INNER JOIN storyboard_frames sf ON sf.frame_id = f.id AND sf.storyboard_id = " . (int)$storyboardId;
    }

    $where  = "f.entity_type = " . $pdo->quote($entityType);

    // Only enforce searchable if we are browsing generally. If a specific frame/storyboard/run is requested, bypass it.
    if ($entityType === 'sketches' && !$hasFrameFilters) {
    ...
    if ($frameId) {
        $where .= " AND f.id = " . (int)$frameId;
    }

    if ($hasEntityFilters) {
        if (empty($intersectIds)) {
            _respond('list_frames', [], 0, $page, 0, $entityType,
                     _buildFilterSummary($fuzzIds, $docId, $docEntityName, $docEntityNames,
                                         $kgNodeId, $seqId, $storyboardId, $vectorText, $mapRunId, $entityId, $frameId, $searchTerms));
            return;
        }
    ...
    // ── ORDER BY ──────────────────────────────────────────────────────
    if ($storyboardId) {
        $orderBy = "sf.sort_order ASC, sf.id ASC";
    } else {
        $orderBy = match($sort) {
    ...
    // ── COUNT ─────────────────────────────────────────────────────────
    $countSql = "SELECT COUNT(*) FROM frames f $joinSql WHERE $where";
    ...
    if ($inclMembership) {
        $memberCols = ",
            CASE WHEN EXISTS (
                SELECT 1 FROM storyboard_frames tsf WHERE tsf.frame_id = f.id LIMIT 1
...
function _listEntities(
    ...
    $intersectIds = computeForgeIntersection(
        $pdo, $entityType,
        $fuzzIds, $docId, $docEntityName, $docEntityNames,
        $kgNodeId, $seqId, $storyboardId, $vectorText,
        $mapRunId, $entityId, $frameId, $filterMode
    );
...
function _buildFilterSummary(
    ...
    $summary = [];
    if (!empty($fuzzIds))      $summary[] = ['type' => 'fuzz',       'ids' => $fuzzIds];
    ...
    if ($frameId)              $summary[] = ['type' => 'frame_id',   'id' => $frameId];
    if (!empty($searchTerms))  $summary[] = ['type' => 'search',     'terms' => $searchTerms];
    return $summary;
}
