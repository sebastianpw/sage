...
// Usage:
//   Interactive: php public/cli_autopilot_db.php
//   Batch:       php public/cli_autopilot_db.php [GenID] [Amount]
//   JSON:        php public/cli_autopilot_db.php --config=job.json
//   JSON inline: php public/cli_autopilot_db.php --config-json='{"desc_gen_id":12,"amount":5,...}'
//   Queue:       php public/cli_autopilot_db.php --qjobs=3
...
try {
    ...
    $jobsToProcess = [];
    if ($qjobs > 0) {
        $jobsToProcess = $conn->fetchAllAssociative("SELECT * FROM forge_jobs WHERE job_type = 'autopilot' AND status = 'pending' ORDER BY priority ASC, id ASC LIMIT " . $qjobs);
        if (empty($jobsToProcess)) {
            echo "No pending autopilot jobs found.\n";
            exit(0);
        }
    } else {
    ...
    foreach ($jobsToProcess as $jobRow) {
        ...
        if ($jobId) {
            ...
            $dbPayload = json_decode($jobRow['payload'], true) ?: [];
            
            $cfg = [
                'desc_gen_id' => $dbPayload['desc_gen_id'] ?? null,
                'amount' => (int)($dbPayload['amount'] ?? 0),
                'probabilities' => normalizeProbabilityConfig(
                    $dbPayload['probabilities'] ?? [], 
                    array_combine(array_keys($ingredientsConfig), array_column($ingredientsConfig, 1))
                ),
                'dry_run' => false,
                '_source' => 'queue',
                '_strict' => true
            ];
            echo "\n=== RUNNING QUEUE JOB #$jobId (autopilot) ===\n";
        } else {
        ...
        try {
            ...
            if ($interactive && ($cfg['_source'] === 'interactive')) {
                ...
                if (empty($gens)) {
                    throw new RuntimeException("No generators found tagged with 'rapidcreate'.");
                }
                ...
                if ($descGenId === null || $descGenId <= 0) {
                    if (($cfg['_strict'] ?? false) === true) {
                        throw new RuntimeException("desc_gen_id is required in JSON/DB mode.");
                    }
                    $gens = getRapidGenerators($conn);
                    if (empty($gens)) {
                        throw new RuntimeException("No generators found tagged with 'rapidcreate'.");
            ...
            if (!$descConfig) {
                throw new RuntimeException("Error: Generator ID $descGenId not found.");
            }

            $nameConfig = $repo->findOneBy(['configId' => DEFAULT_NAME_GEN_HASH]);
            if (!$nameConfig) {
                throw new RuntimeException("Error: Default Name Generator (Hash) not found.");
            }
