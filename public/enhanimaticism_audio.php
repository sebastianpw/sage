<?php
// public/enhanimaticism_audio.php
// Enhanimaticism (Audio mode) — Unified Audio Tool (Entity-specific)
// Mirrors enhanimaticism.php architecture but for audio entities:
//   - Entity list panel (Entities mode only; no map-run toggle)
//   - Howler.js player with full controls + waveform seek
//   - Per-entity playlist (audios table via v_player_<entity> view)
//   - Recording (source/result), regen, copy, delete, voice-identity
// ----------------------------------------------------
...
// Audio entities whitelist — only entities that have an audio equivalent
$audioEntities = [
    'audio_ambiences',
    'audio_cues',
    ...
    try {
        ...
        if ($action === 'get_playlist') {
            ...
            if ($viewExists) {
                ...
                // Fallback: direct audios table
                $where  = "entity_type = " . $pdo->quote($reqEntity) . " AND entity_id = " . intval($entityId);
                if ($search) {
                    $s      = $pdo->quote("%" . $search . "%");
                    $where .= " AND filename LIKE $s";
                }
                $sql  = "SELECT id as audio_id, filename, created_at, entity_id FROM audios WHERE $where ORDER BY id DESC LIMIT 200";
                $rows = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
            }
...
// ─── Page render ───────────────────────────────────────
$pageTitle = 'Enhanimaticism — Audio Mode';
ob_start();
?>
<!-- Dependencies -->
...
<div class="eha-layout">
    ...
    <div class="eha-header">
        <div class="eha-title"><span>&#127925;</span> <span style="font-size:0.7em; opacity:0.6; margin-left:8px;">Audio Mode</span></div>
        <div class="eha-nav">
...
const LS_KEY = 'enhanimaticism_audio_nav';
