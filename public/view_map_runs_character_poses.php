<?php
// public/view_map_runs_character_poses.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'regenerate_run') {
    ...
    if ($map_run_id) {
        try {
            // Find all character_poses entity IDs linked to frames in this run
            $stmt = $pdo->prepare("SELECT DISTINCT entity_id FROM frames WHERE map_run_id = :mid AND entity_type = 'character_poses'");
            $stmt->execute(['mid' => $map_run_id]);
            $ids = $stmt->fetchAll(PDO::FETCH_COLUMN);

            if (!empty($ids)) {
                // Update character_poses table
                $inQuery = implode(',', array_fill(0, count($ids), '?'));
                $upStmt = $pdo->prepare("UPDATE character_poses SET regenerate_images = 1 WHERE id IN ($inQuery)");
                $upStmt->execute($ids);
            }
...
// Base Condition
$whereClause = "WHERE mr.entity_type = 'character_poses'";
$params = [];
...
function getFramesForRun($pdo, $map_run_id) {
    ...
    FROM frames f
    LEFT JOIN character_poses cp ON cp.id = f.entity_id AND f.entity_type = 'character_poses'
...
<head>
    ...
    <title>Character Poses Map Runs</title>
...
<body>
    ...
    <div class="header-compact">
        <a href="sql_crud_character_poses.php" class="entity-icon-link" title="Character Poses CRUD"><?php echo $iconChar; ?></a>
        <a href="gallery_character_poses_nu.php" class="entity-icon-link" title="Character Poses Gallery">▦</a>
        <h2 style="margin:0; font-size:1.2rem; color:var(--text);">Character Poses Map Runs</h2>
        <?php if ($schedulerId): ?>
    ...
    <div class="container">
        ...
        <?php else: ?>
            <div class="empty-state">
                <p>No map runs found for Character Poses.</p>
                <?php if($search): ?>
