<?php
// public/view_map_runs_generatives.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'regenerate_run') {
    ...
    if ($map_run_id) {
        try {
            // Find all generative entity IDs linked to frames in this run
            $stmt = $pdo->prepare("SELECT DISTINCT entity_id FROM frames WHERE map_run_id = :mid AND entity_type = 'generatives'");
            $stmt->execute(['mid' => $map_run_id]);
            $ids = $stmt->fetchAll(PDO::FETCH_COLUMN);

            if (!empty($ids)) {
                // Update generatives table
                $inQuery = implode(',', array_fill(0, count($ids), '?'));
                $upStmt = $pdo->prepare("UPDATE generatives SET regenerate_images = 1 WHERE id IN ($inQuery)");
                $upStmt->execute($ids);
            }
...
// Base Condition
$whereClause = "WHERE mr.entity_type = 'generatives'";
$params = [];
...
<head>
    ...
    <title>Generatives Map Runs</title>
...
<body>
    ...
    <div class="header-compact">
        <a href="sql_crud_generatives.php" class="entity-icon-link" title="Generatives CRUD"><?php echo $iconChar; ?></a>
        <a href="gallery_generatives_nu.php" class="entity-icon-link" title="Generatives Gallery">▦</a>
        <h2 style="margin:0; font-size:1.2rem; color:var(--text);">Generatives Map Runs</h2>
        <?php if ($schedulerId): ?>
    ...
    <div class="container">
                                ...
                                <div class="chain-card" 
                                    ...
                                    <div class="chain-card-thumbnail">
                                        <a href="<?= htmlspecialchars($img_path) ?>"
                                           ...
                                           target="_blank">
                                            <img src="<?= htmlspecialchars($img_path) ?>" alt="<?= htmlspecialchars($frame['frame_name']) ?>" loading="lazy" onload="this.parentElement.dataset.pswpWidth = this.naturalWidth; this.parentElement.dataset.pswpHeight = this.naturalHeight;">
                                        </a>
                                    </div>
                                    ...
                                    <div class="chain-card-body">
                                        ...
                                        <div class="chain-card-meta">
                                            ...
                                            <?php if (!empty($frame['generative_name'])): ?>
        ...
        <?php else: ?>
            <div class="empty-state">
                <p>No map runs found for Generatives.</p>
                <?php if($search): ?>
    ...
    <div id="meta-modal" class="modal-overlay">
        <div class="modal-content">
            ...
            <h3 class="modal-title">Generative Meta Information</h3>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        $(document).on('click', '.regen-run-btn', function(e) {
            ...
            if (!confirm("Are you sure you want to mark all generatives in this map run (Run #" + runId + ") for regeneration?")) {
                return;
            }
            ...
            $.post('', { // Post to current file
                ...
                if (response.success) {
                    Toast.show('Success: ' + response.count + ' generative(s) marked for regeneration.', 'success');
