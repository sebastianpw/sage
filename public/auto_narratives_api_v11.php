<?php
// public/auto_narratives_api_v11.php
// Showrunner V11 - Graph-Augmented Auto-Narrative Laboratory API
// ----------------------------------------------------------------
// VERSION: V11 (Hybrid Engine — KG Graph-Walker + SQL Tags + Chroma Vectors)
//
// ARCHITECTURE: Full V10.2 pipeline preserved exactly.
//
// NEW IN V11 — OPTIONAL AUGMENTATION LAYER (all off by default):
//   enable_kg_graph (bool) — walks 1-degree KG edges per entity,
//     appends causal relationship text to lore desc AND injects
//     into AI Director prompt under "KNOWLEDGE GRAPH CAUSAL RELATIONSHIPS".
//   enable_sql_tags (bool) — runs a deterministic SQL tag strike
//     against tags_2_sketches before Chroma sweep; exact-match
//     results are priority-inserted at the front of the pool and
//     given a +0.2 score bump during Stage 2 scoring.
//   enable_chroma (bool, default true) — when false, bypasses all
//     Chroma vector queries; pool is SQL-tags-only; seed scoring
//     and pickNext fall back to simple structural pass.
//
// All three toggles are parsed from filter_payload JSON.
// When all three are at their defaults (kg=off, tags=off, chroma=on)
// the pipeline is byte-for-byte identical to V10.2.
//
// PRESERVED from V10.2 (unchanged):
...
//   - LoreAccessService integration
//   - decodeField() helper
//   - buildRichQueryFromFilter() (now augmented with optional KG edges)
//   - Keyword injection for precision entity names
//   - Two-pass selection (strict + relaxed constraints)
//   - Unrestricted fallback
//   - All GET routes: list_results, get_filter_cats, get_filter_items, get_entity_preview
//   - weightedRandomPick() with temperature control
//   - AI Enricher (filter_query_enricher_v1)
//   - Per-item query union pool (multi-entity precision mode)
//   - serializeEntityField() universal field serializer
//   - flatten_array_values_auto() helper
// ----------------------------------------------------------------
...
// ============================================================
// AUTO DIRECTOR CLASS - V11 (Hybrid Graph-Vector Pipeline)
// ============================================================

class AutoDirectorV11 {
    ...
    // --- V11 Feature Toggles (all default-off except Chroma) ---
    private $enableKgGraph    = false;
    private $enableSqlTags    = false;
    private $enableChroma     = true;
    private $activeGraphEdges = []; // Stores walked edges for the AI Director
    private $kgNeighborNames  = []; // V11: Neighbor entity names from graph-walk (fed to SQL striker)
    private $anchorTagIdsCache = null; // V11: Cached tag IDs representing the worldKeywords
    ...
    private function enrichFilterQuery(string $rawQuery, string $seriesBible, string $keywords): string {
        // We do NOT check $this->useAi here.
    ...
    // V11: Optionally appends KG edge text per entity when
    // enable_kg_graph is true.
    // ============================================================
    ...
    private function buildRichQueryFromFilter($filterItems, $freeText, $docId) {
        ...
        foreach ($filterItems as $item) {
            ...
            // --------------------------------------------------------
            if ($this->enableKgGraph) {
                $kgStmt = $this->pdo->prepare(
                ...
                if ($kgNodeId) {
                    ...
                    // Outgoing edges (items this node links to)
                    $outStmt = $this->pdo->prepare(
                        "SELECT item_type, item_label, relationship, note
                         FROM kg_node_items
                         WHERE node_id = ?
                    ...
                    foreach ($outStmt->fetchAll(PDO::FETCH_ASSOC) as $edge) {
                        ...
                        // V11: Collect neighbor name for SQL striker expansion
                        if (!empty($edge['item_label'])) {
                            $this->kgNeighborNames[] = $edge['item_label'];
                        }
                    }

                    // Incoming edges (other nodes that link to this one)
                    ...
                    foreach ($inStmt->fetchAll(PDO::FETCH_ASSOC) as $edge) {
                        ...
                        // V11: Collect neighbor name for SQL striker expansion
                        if (!empty($edge['name'])) {
                            $this->kgNeighborNames[] = $edge['name'];
                        }
                    }

                    if (!empty($edgeLines)) {
                        $edgeText = "\nKnowledge Graph Connections:\n" . implode("\n", $edgeLines) . ". ";
                        $this->activeGraphEdges[] = "Entity: $name\n" . implode("\n", $edgeLines);
                        $this->log("V11 Graph-Walk successful for '$name': " . count($edgeLines) . " edges mapped, " . count($this->kgNeighborNames) . " neighbor names harvested for SQL striker.");
                    }
                }
            }
            ...
            if ($found && $desc) {
                ...
                if ($edgeText) $plain .= $edgeText;
                $queryParts[] = $plain;
                $this->perItemQueries[] = $plain;
            }
            
            unset($edgeText); // Reset for next iteration
        }
    ...
    // ============================================================
    // STAGE 1: BUILD HYBRID SKETCH POOL
    // ============================================================
    // V11: Union of SQL Tag exact matches (optional) + Chroma sweep
    // (optional). SQL results are priority-inserted at the front.
    // When Chroma is disabled, pool is SQL-only.
    // When both are disabled, pool is empty (unrestricted fallback).
    // ============================================================
    ...
    private function buildLorePool($richQuery, $currentSketch = null, $stepIndex = 0) {
        ...
        // --------------------------------------------------------
        $sqlPoolIds = [];
        if ($this->enableSqlTags && ($this->precisionMode || !empty($this->kgNeighborNames))) {
            $searchTerms = [];
            if ($this->precisionMode && !empty($this->precisionEntityNames)) {
                foreach ($this->precisionEntityNames as $pn) {
                    $clean = mb_substr(mb_strtolower(trim(preg_replace('/[^a-zA-Z0-9\-\s]/u', '', $pn))), 0, 50);
                    if (strlen($clean) > 2) {
                        $searchTerms[] = $clean;
                        // Split into individual words to cast a wider net
                        $words = explode(' ', $clean);
                        if (count($words) > 1) {
                            foreach ($words as $w) {
                                if (strlen($w) > 3) $searchTerms[] = $w;
                            }
                        }
                    }
                }
            ...
            // V11: When KG graph-walk is active, also strike on neighbor names
            if ($this->enableKgGraph && !empty($this->kgNeighborNames)) {
                foreach ($this->kgNeighborNames as $nn) {
                    $clean = mb_substr(mb_strtolower(trim(preg_replace('/[^a-zA-Z0-9\-\s]/u', '', $nn))), 0, 50);
                    if (strlen($clean) > 2) {
                        $searchTerms[] = $clean;
            ...
            if (!empty($searchTerms)) {
                $this->log("Stage 1: V11 SQL Tag Striker engaged with " . count($searchTerms) . " terms.");
                
                // Build robust LIKE query to catch partial tags
                $likeClauses = [];
                $params = [];
                foreach ($searchTerms as $term) {
                    $likeClauses[] = "t.name LIKE ?";
                    $params[] = '%' . $term . '%';
                }
                ...
                $tagStmt = $this->pdo->prepare("
                    SELECT DISTINCT t2s.to_id
                    FROM tags_2_sketches t2s
                    JOIN tags t ON t.id = t2s.from_id
                    JOIN sketches s ON s.id = t2s.to_id
                    WHERE ($whereOr) AND s.searchable = 1
                    LIMIT 300
                ");
                $tagStmt->execute($params);
                $sqlPoolIds = $tagStmt->fetchAll(PDO::FETCH_COLUMN);
                $this->log("Stage 1: SQL Tags found " . count($sqlPoolIds) . " sketch matches via LIKE.");
            }
        }

        // --------------------------------------------------------
        // V11: CHROMA SWEEP (preserved from V10, conditional)
        // --------------------------------------------------------
        if ($this->enableChroma) {
            ...
            $this->log("Stage 1: Chroma Sweep disabled by configuration.");
        }

        // Merge SQL exact matches into the front of the pool (priority position)
        foreach ($sqlPoolIds as $sid) {
            $id = (int)$sid;
            if ($id > 0 && !isset($seen[$id])) {
                array_unshift($poolIds, $id);
                $seen[$id] = true;
            }
        }
        ...
        $this->log("Stage 1 Hybrid Pool built: " . count($poolIds) . " sketches (SQL=" . count($sqlPoolIds) . " Chroma=" . ($this->enableChroma ? 'on' : 'off') . ").");
        return $poolIds;
    }
    ...
    private function askAiForNextShot($current, $richQuery, $mode, $progress, $seedQuery) {
        ...
        // V11: Inject KG causal relationship data into the Director prompt
        if ($this->enableKgGraph && !empty($this->activeGraphEdges)) {
            $inputText .= "\n=== KNOWLEDGE GRAPH CAUSAL RELATIONSHIPS ===\n";
            $inputText .= implode("\n\n", $this->activeGraphEdges) . "\n";
        }
    ...
    public function generate($docId, $docName, $mode, $targetLength, $filterPayload) {
        ...
        $this->activeGraphEdges     = []; // V11: Reset for this run
        $this->kgNeighborNames      = []; // V11: Reset for this run
        $this->anchorTagIdsCache    = null; // Reset cache

        $this->docId   = $docId;
        $this->docName = $docName;

        // V11: Parse feature toggles from filter payload
        $filter = json_decode($filterPayload, true) ?: [];
        $this->enableKgGraph = isset($filter['enable_kg_graph']) ? (bool)$filter['enable_kg_graph'] : false;
        $this->enableSqlTags = isset($filter['enable_sql_tags']) ? (bool)$filter['enable_sql_tags'] : false;
        $this->enableChroma  = isset($filter['enable_chroma'])   ? (bool)$filter['enable_chroma']   : true;

        $this->log("Director Mode: " . strtoupper($mode));
        $this->log("V11 Engine Config -> Graph-Walker: " . ($this->enableKgGraph ? 'ON' : 'OFF') . " | SQL Tags: " . ($this->enableSqlTags ? 'ON' : 'OFF') . " | Chroma: " . ($this->enableChroma ? 'ON' : 'OFF'));
        if ($this->useAi)     $this->log("AI Reasoning: ON");
        if ($this->debugMode) $this->log("[DEBUG] Debug mode active. Stage 1→2 pipeline (V11).");
    ...
    private function pickSeed() {
        ...
        if (!$this->enableChroma) {
            $this->log("Chroma Disabled: Picking seed randomly from SQL pool.");
            return $this->getFullSketchData($poolIds[array_rand($poolIds)]);
        }
        ...
        if ($this->enableSqlTags) {
            foreach (array_slice($poolIds, 0, 20) as $sqlId) {
                if (isset($scores[$sqlId])) $scores[$sqlId] += 0.2;
            }
        }
    ...
    private function pickSeedDirect() {
        if (!$this->enableChroma) return null;
        ...
        if (!$this->enableChroma) {
            if ($usePool) {
                ...
                foreach ($poolIds as $pid) {
                    $cand = $this->getFullSketchData($pid);
                    if (!$cand) continue;
                    ...
                    if (($cand['standalone'] ?? 'yes') !== 'needs-context' || ($current['standalone'] ?? 'yes') !== 'needs-context') {
                        $relaxed[] = $cand;
                    }
                }
                
                if (!empty($valid)) {
                    $picked = $valid[array_rand($valid)];
                    $this->log("Shot {$stepIndex}: Picked strict match from SQL pool.");
                    return $picked;
                }
                if (!empty($relaxed)) {
                    $this->log("Shot {$stepIndex}: Strict constraints exhausted SQL pool. Relaxing rules.");
                    return $relaxed[array_rand($relaxed)];
                }
                if (!empty($poolIds)) {
                    $this->log("Shot {$stepIndex}: Constraints exhausted SQL pool. Using random pool item.");
                    return $this->getFullSketchData($poolIds[array_rand($poolIds)]);
                }
            }
            
            $this->log("Shot {$stepIndex}: SQL pool empty. Triggering associative SQL fallback.");
            return $this->pickNextUnrestrictedSql($current, $excludeIds, $progress);
        }
    ...
    // --------------------------------------------------------
    // HELPER: GET ANCHOR TAG IDS
    // Resolves documentations.keywords into DB Tag IDs via LIKE
    // --------------------------------------------------------
    private function getAnchorTagIds() {
        if ($this->anchorTagIdsCache !== null) return $this->anchorTagIdsCache;
        $this->anchorTagIdsCache = [];
        
        if (empty($this->worldKeywords)) return [];

        $kwArray = explode(',', $this->worldKeywords);
        $likeClauses = [];
        $params = [];
        
        foreach ($kwArray as $w) {
            $clean = mb_substr(mb_strtolower(trim(preg_replace('/[^a-zA-Z0-9\-\s]/u', '', $w))), 0, 50);
            if (strlen($clean) > 2) {
        ...
        if (empty($likeClauses)) return [];

        $whereOr = implode(' OR ', $likeClauses);
        $stmt = $this->pdo->prepare("SELECT id FROM tags WHERE $whereOr");
        $stmt->execute($params);
        $this->anchorTagIdsCache = $stmt->fetchAll(PDO::FETCH_COLUMN);
        
        return $this->anchorTagIdsCache;
    }

    // --------------------------------------------------------
    // UNRESTRICTED FALLBACK (SQL ONLY - NO CHROMA)
    // --------------------------------------------------------
    private function pickNextUnrestrictedSql($current, $excludeIds, $progress) {
        $excludeSql = '';
        if (!empty($excludeIds)) {
            $excludeSql = "AND s.id NOT IN (" . implode(',', array_map('intval', $excludeIds)) . ")";
        }

        $rows = [];
        $currentId = (int)$current['id'];
        
        // 1. Tag-Walking + Gravitational Anchor
        $tagStmt = $this->pdo->prepare("SELECT from_id FROM tags_2_sketches WHERE to_id = ?");
        $tagStmt->execute([$currentId]);
        $currentTagIds = $tagStmt->fetchAll(PDO::FETCH_COLUMN);
        
        $anchorTagIds = $this->getAnchorTagIds();
        $combinedTagIds = array_values(array_unique(array_merge($currentTagIds, $anchorTagIds)));
        
        if (!empty($combinedTagIds)) {
            $inTags = implode(',', array_map('intval', $combinedTagIds));
            
            $sql = "
                SELECT s.id, COUNT(t2s.from_id) as shared_tags
                FROM sketches s
                JOIN tags_2_sketches t2s ON s.id = t2s.to_id
                JOIN sketch_sequence_analysis ssa ON s.id = ssa.sketch_id
                WHERE t2s.from_id IN ($inTags) AND s.searchable = 1 $excludeSql
                GROUP BY s.id
            ...
            if (!empty($rows)) {
                $this->log("SQL Fallback: Found " . count($rows) . " sketches via Tag-Walking (Current + Anchor).");
            }
        }
        
        // 2. World-Bounds Fallback
        if (empty($rows) && $this->docId) {
            $docId = (int)$this->docId;
            $sql = "
                SELECT DISTINCT s.id
                FROM sketches s
                JOIN sketch_lore_history slh ON s.id = slh.sketch_id
                JOIN sketch_sequence_analysis ssa ON s.id = ssa.sketch_id
                WHERE slh.doc_id = $docId AND s.searchable = 1 $excludeSql
                ORDER BY RAND()
                LIMIT 50
            ";
            $rows = $this->pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
            if (!empty($rows)) {
                $this->log("SQL Fallback: Tag-walk failed. Found " . count($rows) . " sketches in same Context Doc.");
            }
        }
        ...
        if (empty($rows)) {
            $sql = "
                ...
                FROM sketches s
                JOIN sketch_sequence_analysis ssa ON s.id = ssa.sketch_id
                WHERE s.searchable = 1 $excludeSql
                ORDER BY RAND() LIMIT 50
            ";
            $rows = $this->pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
            $this->log("SQL Fallback: Using global random sketches.");
        }
        
        // Guaranteed selection passes
        foreach ($rows as $r) {
            $cand = $this->getFullSketchData($r['id']);
            if ($cand && $this->checkStructuralConstraints($current, $cand, $progress)) return $cand;
        }
        
        foreach ($rows as $r) {
            $cand = $this->getFullSketchData($r['id']);
            if (!$cand) continue;
            if (($cand['standalone'] ?? 'yes') === 'needs-context' && ($current['standalone'] ?? 'yes') === 'needs-context') continue;
            return $cand;
        }
...
// ============================================================
// API CONTROLLER (all routes preserved from V10.2)
// ============================================================
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'generate_sequence') {
        try {
            ...
            $director = new AutoDirectorV11($pdo, $useAi, $debugMode);
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // --------------------------------------------------------
    // ENTITY PREVIEW (Peek Modal) — preserved V11
    // --------------------------------------------------------
