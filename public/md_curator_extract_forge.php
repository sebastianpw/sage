<?php
// public/md_curator_extract_forge.php
// Mobile-first JSON builder for MD Curator Extract
// Writes config for:
//   php public/cli_md_curator_extract_json.php --config=md_curator_job.json
...
function getGeneratorLabel($repo, string $configId): array {
    $row = $repo->findOneBy(['configId' => $configId]);
    if (!$row) {
        return ['id' => $configId, 'title' => $configId, 'model' => 'unknown'];
    }
    return [
        'id' => $row->getConfigId(),
        'title' => $row->getTitle(),
        'model' => $row->getModel() ?: 'openai',
    ];
}
...
if (isset($_GET['action']) && $_GET['action'] === 'meta') {
    ...
    echo json_encode([
        ...
        'fixed_configs' => [
            'curator' => getGeneratorLabel($repo, $curatorConfigId),
            'showrunner' => getGeneratorLabel($repo, $showrunnerConfigId),
        ],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_json') {
    ...
    echo json_encode([
        ...
        'saved' => true,
        'path' => $file,
        'runner' => 'php public/cli_md_curator_extract_json.php --config=' . $file,
    ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    exit;
}
...
$defaults = buildDefaults();
$curatorFixed = getGeneratorLabel($repo, $curatorConfigId);
$showrunnerFixed = getGeneratorLabel($repo, $showrunnerConfigId);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<title>MD Curator Extract Forge</title>
...
<header>
    <div class="brand">
        ...
        <span>MD Curator Extract Forge</span>
    </div>
...
<main class="main">
    <div class="shell">
        ...
        <div class="card desktop-span">
            ...
            <div class="card-body">
                <div class="help">
                    Build a JSON preset for the extractor. The JSON runner preserves the original extraction behavior.
                </div>
            </div>
        </div>
        ...
        <details class="section" open>
            <summary class="section-head">
                <div class="section-head-left">
                    ...
                    <div class="section-label">
                        <strong>Scope</strong>
                        <span>Choose category, limit, chunk size, and optional model override.</span>
                    </div>
                </div>
            ...
            <div class="section-content">
                ...
                <div class="inline-row">
                    ...
                    <div class="field">
                        <div class="label">Chars per chunk</div>
                        <input type="number" id="charLimit" min="500" step="100" value="<?= (int)$defaults['charLimit'] ?>">
                    </div>
                </div>

                <div class="field">
                    <div class="label">Override model</div>
                    <input type="text" id="overrideModel" placeholder="leave blank to use config default">
                </div>
            </div>
        </details>
        ...
        <details class="section" open>
            <summary class="section-head">
                <div class="section-head-left">
                    ...
                    <div class="section-label">
                        <strong>Fixed generator configs</strong>
                        <span>These are the two configs used by the extractor.</span>
                    </div>
                </div>
                <div class="section-chevron"><i class="bi bi-chevron-down"></i></div>
            </summary>
            <div class="section-content">
                <div class="meta-chip"><i class="bi bi-cpu"></i> Curator: <?= esc($curatorFixed['title']) ?> (<?= esc($curatorFixed['id']) ?>)</div>
                <div class="meta-chip"><i class="bi bi-diagram-2"></i> Showrunner: <?= esc($showrunnerFixed['title']) ?> (<?= esc($showrunnerFixed['id']) ?>)</div>
            </div>
        </details>
            ...
            <div class="section-content">
                <div class="stats-grid">
                    <div class="stat">
                        ...
                        <div class="stat-meta">
                            <div class="command" id="cmdOut">php public/cli_md_curator_extract_json.php --config=&lt;file.json&gt;</div>
                        </div>
                    </div>
                    <div class="stat">
                        <div class="stat-top">
...
function getPayload() {
    ...
    return {
        ...
        charLimit: parseInt(document.getElementById('charLimit').value || '4000', 10),
...
function updateJson() {
    ...
    document.getElementById('scopeSummary').innerHTML =
        ...
        `<span class="ok">Limit:</span> ${payload.limit}<br>` +
        `<span class="ok">Chars:</span> ${payload.charLimit}<br>` +
        `<span class="ok">Model override:</span> ${esc(payload.overrideModel || '—')}`;
}
...
function bindEvents() {
    ['targetCategoryId','limit','charLimit','overrideModel'].forEach(id => {
...
function init() {
    renderCategories(categories);
    document.getElementById('targetCategoryId').value = String(initialDefaults.targetCategoryId ?? 0);
    document.getElementById('limit').value = String(initialDefaults.limit ?? 10);
    document.getElementById('charLimit').value = String(initialDefaults.charLimit ?? 4000);
    document.getElementById('overrideModel').value = initialDefaults.overrideModel ?? '';
    bindEvents();
    updateJson();
}
