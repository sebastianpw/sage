<?php
// public/view_map_runs_character_anima_poses.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'regenerate_run') {
    ...
    if ($map_run_id) {
        try {
            // Find all character_anima_poses entity IDs linked to frames in this run
            $stmt = $pdo->prepare("SELECT DISTINCT entity_id FROM frames WHERE map_run_id = :mid AND entity_type = 'character_anima_poses'");
            $stmt->execute(['mid' => $map_run_id]);
            $ids = $stmt->fetchAll(PDO::FETCH_COLUMN);

            if (!empty($ids)) {
                // Update character_anima_poses table
                $inQuery = implode(',', array_fill(0, count($ids), '?'));
                $upStmt = $pdo->prepare("UPDATE character_anima_poses SET regenerate_images = 1 WHERE id IN ($inQuery)");
                $upStmt->execute($ids);
            }
...
// Base Condition
$whereClause = "WHERE mr.entity_type = 'character_anima_poses'";
$params = [];
...
// Icon Selection
$entity = 'character_anima_poses';
$iconChar = $entityIcons[$entity] ?? '📦';
$schedulerId = $entitySchedulerIds[$entity] ?? null;

// --- 2. UI Module Setup ---
$registry = ModuleRegistry::getInstance();
$entities_with_menu = ['characters', 'character_anima_poses', 'animas', 'locations', 'backgrounds', 'artifacts', 'vehicles', 'scene_parts', 'controlnet_maps', 'spawns', 'generatives', 'sketches', 'prompt_matrix_blueprints', 'composites'];
...
function getFramesForRun($pdo, $map_run_id) {
    ...
    SELECT 
        ...
        f.entity_type,
        f.entity_id,
        cp.name as character_anima_pose_name,
        ie.tool as edit_tool,
        ie.note as edit_note,
        f.img2img_frame_id
        
    FROM frames f
    LEFT JOIN character_anima_poses cp ON cp.id = f.entity_id AND f.entity_type = 'character_anima_poses'
...
<head>
    ...
    <title>Character Anima Poses Map Runs</title>
...
<body>
    ...
    <div class="header-compact">
        <a href="sql_crud_character_anima_poses.php" class="entity-icon-link" title="Character Anima Poses CRUD"><?php echo $iconChar; ?></a>
        <a href="gallery_character_anima_poses_nu.php" class="entity-icon-link" title="Character Anima Poses Gallery">▦</a>
        <h2 style="margin:0; font-size:1.2rem; color:var(--text);">Character Anima Poses Map Runs</h2>
        <?php if ($schedulerId): ?>
    ...
    <div class="container">
                                ...
                                <div class="chain-card" 
                                    ...
                                    <div class="chain-card-body">
                                        ...
                                        <div class="chain-card-meta">
                                            ...
                                            <?php if (!empty($frame['character_anima_pose_name'])): ?>
                                                 <span class="badge badge-blue"><?= htmlspecialchars($frame['character_anima_pose_name']) ?></span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </div>
        ...
        <?php else: ?>
            <div class="empty-state">
                <p>No map runs found for Character Anima Poses.</p>
                <?php if($search): ?>
    ...
    <div id="meta-modal" class="modal-overlay">
        <div class="modal-content">
            ...
            <h3 class="modal-title">Character Anima Pose Meta Information</h3>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        $(document).on('click', '.regen-run-btn', function(e) {
            ...
            if (!confirm("Are you sure you want to mark all character poses in this map run (Run #" + runId + ") for regeneration?")) {
                return;
            }
