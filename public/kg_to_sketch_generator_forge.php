<?php
// public/kg_to_sketch_generator_forge_mobile.php
// Mobile-first Forge-style JSON builder for the KG -> Sketch generator
...
function payloadFromPost(array $post): array {
    return [
        ...
        'node_type' => trim((string)($post['node_type'] ?? '')),
        'history_filter' => trim((string)($post['history_filter'] ?? 'new')),
...
if (isset($_GET['action']) && $_GET['action'] === 'meta') {
    ...
    $categories = $conn->fetchAllAssociative("SELECT id, name FROM kg_categories ORDER BY name ASC");
    $categoriesOut = [];
    ...
    foreach ($categories as $cat) {
        ...
        $hist = empty($names) ? 0 : (int)$conn->fetchOne(
            "SELECT COUNT(DISTINCT entity_name) FROM sketch_lore_history WHERE entity_name IN (" . implode(',', array_fill(0, count($names), '?')) . ")",
            $names
        );
        $categoriesOut[] = [
            'id' => $catId,
    ...
    if ($mode === 1) {
        if ($categoryId > 0) {
            ...
            $types = $conn->fetchAllAssociative(
                "SELECT node_type, COUNT(*) as cnt FROM kg_nodes WHERE status = 'active' AND category_id IN ($familySql) GROUP BY node_type ORDER BY cnt DESC"
            );
        } else {
            $types = $conn->fetchAllAssociative(
                "SELECT node_type, COUNT(*) as cnt FROM kg_nodes WHERE status = 'active' GROUP BY node_type ORDER BY cnt DESC"
            );
        }
    } else {
        $types = $conn->fetchAllAssociative(
    ...
    $typeRows = [];
    foreach ($types as $t) {
        $type = (string)$t['node_type'];
        if ($mode === 1 && $categoryId > 0) {
            $family = getKgCatFamily($conn, $categoryId);
            $familySql = implode(',', array_map('intval', $family));
            $nodeNames = $conn->fetchFirstColumn(
                "SELECT name FROM kg_nodes WHERE status = 'active' AND category_id IN ($familySql) AND node_type = ?",
                [$type]
            );
        } else {
            $nodeNames = $conn->fetchFirstColumn(
        ...
        $hc = getHistCounts($conn, $nodeNames ?: [], $type);
        $typeRows[] = [
            'node_type' => $type,
            'count' => (int)$t['cnt'],
            'new' => $hc['new'],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_json') {
    ...
    $dir = __DIR__ . '/../var/kg_sketch_configs';
    if (!is_dir($dir)) {
        @mkdir($dir, 0775, true);
    }

    $name = preg_replace('/[^a-zA-Z0-9_\-]+/', '_', trim((string)($_POST['name'] ?? 'kg_sketch_job')));
    ...
    echo json_encode([
        ...
        'saved' => true,
        'path' => $file,
        'runner' => 'php public/cli_kg_to_sketch_generator_json.php --config=' . $file,
    ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    exit;
}
...
<title>KG Sketch JSON Forge</title>
...
.text-btn.danger{background:var(--red);border-color:transparent;color:#fff;font-weight:700}
...
<header>
    <div class="brand">
        ...
        <span>KG Sketch JSON Forge</span>
    </div>
...
<main class="main">
    <div class="shell">
        ...
        <div class="card desktop-span">
            ...
            <div class="card-body">
                <div class="help">
                    Build a JSON preset here, then feed it into the CLI runner. The layout is optimized for one-handed Android use.
                </div>
            </div>
        </div>
        ...
        <details class="section" open>
            <summary class="section-head">
                <div class="section-head-left">
                    ...
                    <div class="section-label">
                        <strong>Mode and Scope</strong>
                        <span>Choose category-based or global type processing.</span>
                    </div>
                </div>
            ...
            <div class="section-content">
                <div class="field">
                    ...
                    <div class="pill-row">
                        <label class="pill"><input type="radio" name="mode" value="1" checked> By Category</label>
                        <label class="pill"><input type="radio" name="mode" value="2"> By Node Type</label>
                    </div>
                </div>

                <div class="field" id="categoryBlock">
                    <div class="label">Knowledge Graph Category</div>
                    <select id="category_id">
                        <option value="0">-- ALL KNOWLEDGE GRAPH CATEGORIES --</option>
                        <?php foreach ($categories as $cat): ?>
                            <option value="<?= (int)$cat['id'] ?>"><?= htmlspecialchars($cat['name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                    <div class="help">Pick a category to narrow the node pool. In “all” mode, the category stays at zero.</div>
                </div>

                <div class="field">
                    <div class="label">Node Type</div>
                    <select id="node_type"></select>
                    <div class="help" id="typeInfo">Choose the node type to process.</div>
                </div>
            </div>
        </details>
        ...
        <details class="section" open>
            <summary class="section-head">
                <div class="section-head-left">
                    ...
                    <div class="section-label">
                        <strong>Filter and Range</strong>
                        <span>Control history filter, offset, amount, and timing.</span>
                    </div>
                </div>
                <div class="section-chevron"><i class="bi bi-chevron-down"></i></div>
            </summary>
            <div class="section-content">
                <div class="inline-row">
                    <div class="field">
                        <div class="label">History Filter</div>
                ...
                <div class="field">
                    ...
                    <div class="help">The CLI runner pauses between sketches. On mobile, keep this visible and editable.</div>
                </div>
        ...
        <details class="section" open>
            ...
            <div class="section-content">
                <div class="field">
                    ...
                    <select id="generator_config_id">
                        <?php foreach ($generatorConfigs as $gc): ?>
                            <option value="<?= htmlspecialchars($gc['config_id']) ?>"><?= htmlspecialchars($gc['title']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="field">
                    ...
                    <input type="text" id="tags" placeholder="sketch, lore, character">
                    <div class="help">Comma-separated. Use the same tags you want linked to the generated sketches.</div>
                </div>
            </div>
        </details>
            ...
            <summary class="section-head">
                <div class="section-head-left">
                    ...
                    <div class="section-label">
                        ...
                        <span>Counts and available node types update live.</span>
                    </div>
                </div>
                <div class="section-chevron"><i class="bi bi-chevron-down"></i></div>
            </div>
            <div class="section-content">
                <div class="stats-grid">
                    ...
                    <div class="stat">
                        ...
                        <div class="stat-meta">
                            <div class="command" id="cmdOut">php public/cli_kg_to_sketch_generator_json.php --config=&lt;file.json&gt;</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="stat">
                    <div class="stat-top">
                        ...
                        <div class="stat-badge" id="catCountBadge">—</div>
                    </div>
                    <div class="stat-meta">
                ...
                <div class="stat">
                    <div class="stat-top">
                        ...
                        <div class="stat-badge" id="typeCountBadge">—</div>
                    </div>
                    <div class="stat-meta">
...
function getPayload() {
    ...
    return {
        mode: parseInt(document.querySelector('input[name="mode"]:checked').value, 10),
        category_id: parseInt(document.getElementById('category_id').value || '0', 10),
        node_type: document.getElementById('node_type').value || '',
        history_filter: document.getElementById('history_filter').value,
...
function updateJson() {
    ...
    const modeLabel = payload.mode === 1 ? 'By Category' : 'By Node Type';
    document.getElementById('scopeBadge').textContent = modeLabel;
    document.getElementById('scopeSummaryShort').textContent = modeLabel;

    const catText = document.getElementById('category_id').selectedOptions[0]?.text || '';
    const typeText = payload.node_type || '—';
    const tagText = payload.tags.length ? payload.tags.join(', ') : '—';
    const amountText = payload.amount === null ? 'all remaining' : payload.amount;

    document.getElementById('scopeSummary').innerHTML =
        `<span class="ok">Mode:</span> ${esc(modeLabel)}<br>` +
        `<span class="ok">Category:</span> ${esc(catText)}<br>` +
        `<span class="ok">Type:</span> ${esc(typeText)}<br>` +
        `<span class="ok">Filter:</span> ${esc(payload.history_filter)}<br>` +
    ...
    document.getElementById('cmdOut').textContent =
        'php public/cli_kg_to_sketch_generator_json.php --config=/path/to/generated.json';
}

async function loadMeta() {
    const mode = parseInt(document.querySelector('input[name="mode"]:checked').value, 10);
    const categoryId = parseInt(document.getElementById('category_id').value || '0', 10);

    const res = await fetch(`?action=meta&mode=${mode}&category_id=${categoryId}`);
    const data = await res.json();
    if (!data.ok) return;

    const catBox = document.getElementById('categoryStats');
    const typeBox = document.getElementById('typeStats');

    document.getElementById('catCountBadge').textContent = `${data.categories.length}`;
    document.getElementById('typeCountBadge').textContent = `${data.types.length}`;

    catBox.innerHTML = data.categories.map(c => `
        <div class="stat">
            <div class="stat-top">
                <div class="stat-name">${esc(c.name)}</div>
                <div class="stat-badge">#${c.id}</div>
            </div>
            <div class="stat-meta">
                total: ${c.total} · <span class="ok">new: ${c.new}</span> · <span class="warn">hist: ${c.hist}</span>
            </div>
        </div>
    `).join('');

    if (!data.types.length) {
        typeBox.innerHTML = `<div class="stat"><div class="stat-meta">No types found.</div></div>`;
        document.getElementById('node_type').innerHTML = '';
        document.getElementById('typeInfo').textContent = 'No node types available for this scope.';
        updateJson();
        return;
    }

    const select = document.getElementById('node_type');
    const previous = select.value;
    select.innerHTML = data.types.map(t => `<option value="${esc(t.node_type)}">${esc(t.node_type)} (${t.count})</option>`).join('');
    ...
    typeBox.innerHTML = data.types.map(t => `
        <div class="stat">
            <div class="stat-top">
                <div class="stat-name">${esc(t.node_type)}</div>
                <div class="stat-badge">${t.count}</div>
            </div>
            <div class="stat-meta">
                <span class="ok">new: ${t.new}</span> · <span class="warn">hist: ${t.hist}</span>
            </div>
        </div>
    `).join('');

    document.getElementById('typeInfo').textContent =
        `${data.types.length} node type(s) available for the current scope.`;
...
async function savePreset() {
    const name = prompt('Preset name:', 'kg_sketch_job');
    if (name === null) return;
...
function bindEvents() {
    document.querySelectorAll('input[name="mode"]').forEach(el => {
        el.addEventListener('change', async () => {
            const mode = parseInt(document.querySelector('input[name="mode"]:checked').value, 10);
            document.getElementById('categoryBlock').style.display = mode === 1 ? 'block' : 'none';
            await loadMeta();
        });
    });

    ['category_id','node_type','history_filter','offset','amount','generator_config_id','tags','confirm','delay_us','dry_run']
        .forEach(id => {
            document.getElementById(id).addEventListener(
                /checkbox/.test(document.getElementById(id).type) ? 'change' : 'input',
                updateJson
            );
        });
...
function init() {
    ...
    document.getElementById('history_filter').value = initialPayload.history_filter;
    ...
    document.getElementById('generator_config_id').value = initialPayload.generator_config_id;
