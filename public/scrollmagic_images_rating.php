<?php
// public/scrollmagic_images_rating.php
...
// --- Whitelists ---
$sortableFrameColumns = ['id', 'name', 'filename', 'created_at', 'rating'];
...
// CHANGED: Default Sort to created_at DESC
$sortByInput = trim($_GET['sort_by'] ?? 'frames.created_at');
...
// 1. RAW QUERY MODE (Rating ignored here as user provides full SQL)
if (!empty($rawQuery)) {
...
elseif (!empty($storyboardIds) || in_array($assignmentLogic, ['global_multi', 'global_any', 'selected_any', 'selected_all', 'selected_multi'])) {
    ...
    // NEW: Apply Rating Filter to Aggregation Mode
    ...
    // Sorting
    // Note: If sorting by date, we might need f.created_at in SELECT, but standard usage relies on ID or Sort Logic
    // Since default is now created_at, ensure we respect it.
    if ($orderBy === 'usage') {
        $orderSql = "ORDER BY 2 DESC, f.created_at DESC"; 
    } else {
        $sortByParts = explode('.', $sortByInput);
        if (count($sortByParts) !== 2) $sortByParts = ['frames', 'created_at'];
        list($sortTable, $sortColumn) = $sortByParts;
        
        // Only allow sorting by frames columns in this aggregation view to keep it simple
        // or usage logic overrides it
        $orderSql = "ORDER BY f.`{$sortColumn}` {$sortOrder}";
    }

    $dataSql = "SELECT f.id, f.filename, f.name, f.entity_type, f.entity_id, f.created_at, f.rating, {$selectExpr}, {$namesExpr}
                {$fromAgg} 
                {$whereClauseAgg} 
                GROUP BY f.id 
                {$havingClause} 
...
// 3. STANDARD MODE
    ...
    $select = "SELECT DISTINCT f.id, f.filename, f.name, f.entity_type, f.entity_id, f.rating";
    $from = "FROM `frames` f";
    $joins = [];
    $where = [];
    $orderBySql = '';

    // NEW: Apply Rating Filter to Standard Mode
    if ($ratingFilter !== null) {
        $where[] = "f.rating = ?";
        $params[] = $ratingFilter;
    }
...
foreach ($rows as $row) {
    ...
    $images[] = [
        ...
        'storyboard_names' => $row['storyboard_names'] ?? '',
