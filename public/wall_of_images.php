<?php
// public/wall_of_images.php
require_once __DIR__ . '/bootstrap.php';
require __DIR__ . '/env_locals.php';

use App\Core\SpwBase;
use App\Gallery\WallOfImagesGallery;
use App\UI\Modules\ModuleRegistry;

$spw = SpwBase::getInstance();
$entity = "wall_of_images";

// Get module registry
$registry = ModuleRegistry::getInstance();

// Configure gear menu module - will work for ALL entity types in the wall
// IMPORTANT: exclude_entities means NO gear icons on grid items
// We only use the gear menu in PhotoSwipe mode for the wall
$gearMenu = $registry->create('gear_menu', [
    'position' => 'top-right',
    'icon' => '&#9881;',
    'icon_size' => '2em',
    'exclude_entities' => ['wall_of_images'], // Don't show gear icons on wall grid items
]);

// Define entity types that appear in wall of images
$wallEntityTypes = ['characters', 'generatives', 'sketches', 'animas', 'locations', 'backgrounds', 
                    'artifacts', 'vehicles', 'scene_parts', 'controlnet_maps', 'spawns', 
                    'prompt_matrix_blueprints', 'composites', 'character_poses'];

// Add the same actions for ALL entity types that can appear in the wall
foreach ($wallEntityTypes as $entityType) {
    $gearMenu->addAction($entityType, [
        'label' => 'View Frame',
        'icon' => '👁️',
        'callback' => 'window.showFrameDetailsModal(frameId);'
    ]);

    $gearMenu->addAction($entityType, [
        'label' => 'Import to Generative',
        'icon' => '⚡',
        'callback' => 'window.importGenerative(entity, entityId, frameId);'
    ]);

    $gearMenu->addAction($entityType, [
        'label' => 'Edit Entity',
        'icon' => '✏️',
        'callback' => 'window.showEntityFormInModal(entity, entityId);'
    ]);

    $gearMenu->addAction($entityType, [
        'label' => 'Edit Image',
        'icon' => '🖌️',
        'callback' => 'const $w = $(wrapper); ImageEditorModal.open({ entity: entity, entityId: entityId, frameId: frameId, src: $w.find(\'img\').attr(\'src\') });'
    ]);

    
    $gearMenu->addAction($entityType, [
            'label' => 'View Frame Chain',
            'icon' => '🔗',
            'callback' => 'window.showFrameChainInModal(frameId);'
        ]);

        $gearMenu->addAction($entityType, [
            'label' => 'Add to Storyboard',
            'icon' => '🎬',
            'callback' => 'window.selectStoryboard(frameId, $(wrapper));'
        ]);
    
        
    

    $gearMenu->addAction($entityType, [
        'label' => 'Assign to Composite',
        'icon' => '🧩',
        'callback' => 'window.showImportEntityModal({ source: entity, target: "composites", source_entity_id: entityId, frame_id: frameId, limit: 1, copy_name_desc: 0, composite: 1 });'
    ]);

    $gearMenu->addAction($entityType, [
        'label' => 'Import to ControlNet Map',
        'icon' => '☠️',
        'callback' => 'window.importControlNetMap(entity, entityId, frameId);'
    ]);

    $gearMenu->addAction($entityType, [
        'label' => 'Use Prompt Matrix',
        'icon' => '🌌',
        'callback' => 'window.usePromptMatrix(entity, entityId, frameId);'
    ]);

    $gearMenu->addAction($entityType, [
        'label' => 'Delete Frame',
        'icon' => '🗑️',
        'callback' => 'window.deleteFrame(entity, entityId, frameId);'
    ]);
}

// Configure image editor module
$imageEditor = $registry->create('image_editor', [
    'modes' => ['mask', 'crop'],
    'show_transform_tab' => true,
    'show_filters_tab' => true,
    'enable_rotate' => true,
    'enable_resize' => true,
    'preset_filters' => [
        'grayscale', 'vintage', 'sepia', 'clarendon',
        'gingham', 'moon', 'lark', 'reyes', 'juno', 'slumber'
    ],
]);

// Create the gallery instance
$gallery = new WallOfImagesGallery();
$gallery->setGearMenu($gearMenu);
$gallery->setImageEditor($imageEditor);

// Load the frame details modal
ob_start();
require __DIR__ . '/modal_frame_details.php';
$frameDetailsModal = ob_get_clean();

// Render everything together
$content = '<script src="/js/gear_menu_globals.js"></script>'
         . $gearMenu->render()
         . $imageEditor->render()
         . $frameDetailsModal
         . $gallery->render();

$spw->renderLayout(
    $content,
    "Wall of Images",
    $spw->getProjectPath() . '/templates/gallery.php'
);