...
$regenCol = in_array('regenerate_videos', $columns) ? 'regenerate_videos' : (in_array('regenerate', $columns) ? 'regenerate' : null);
...
<div class="header-compact">
    ...
    <a href="view_video_admin.php" class="entity-icon-link" title="Gallery"><?php echo $iconChar; ?></a>

    <div class="header-controls">
        <button id="addBtn" class="btn btn-sm btn-outline-primary">Add</button>
        
        <!-- Scheduler Run Button (ID 42 for Animatics) -->
        <a class="runBtn scheduler" data-id="42" title="Trigger Animatics Scheduler" style="cursor:pointer; font-size:1.2rem; text-decoration:none; margin-left:5px;">🌀</a>
...
function initSlides() {
    ...
    $.post('animatics_crud.php', { action: 'fetch', search: search, page: 1, limit: rowsPerPage }, function(data) {
...
function loadPageData(page) {
    ...
    const search = $('#searchInput').val();
    $.post('animatics_crud.php', { action: 'fetch', search: search, page: page, limit: rowsPerPage }, function(data) {
        data = JSON.parse(data);
...
function loadMapRunsForSlide(slide) {
    slide.find('.mapRunSelect').each(function() {
        ...
        $.post('animatics_crud.php', {action: 'fetchMapRuns', entity_id: id}, function(res){
...
$(document).ready(function() {
    ...
    $('#addBtn').click(function() { $.post('animatics_crud.php', {action:'add'}, function(){ initSlides(); Toast.show('Added','success'); }); });
    ...
    $(document).on('change', '.regen-checkbox', function() {
        ...
        $.post('animatics_crud.php', { action: 'update', id: id, field: field, value: val }, function(res) {
    ...
    $(document).on('click', '.copyBtn', function() { if(confirm('Copy?')) $.post('animatics_crud.php', {action:'copy', id:$(this).closest('tr').data('id')}, function(){ initSlides(); Toast.show('Copied','success'); }); });
    $(document).on('click', '.delete', function() { if(confirm('Delete?')) $.post('animatics_crud.php', {action:'delete', id:$(this).closest('tr').data('id')}, function(){ initSlides(); Toast.show('Deleted','success'); }); });
    ...
    $(document).on('dblclick', 'a.pswp-gallery-item', function(e) {
        ...
        if(confirm('Remove this '+type+' image?')) {
            $.post('animatics_crud.php', {action:'remove_image', id:$(this).data('entity-id'), image_type:type}, function(res){ if(res==='success') { Toast.show('Removed','success'); initSlides(); } });
        }
    });
...
function enableSortable(tbody) {
    ...
    tbody.sortable({ handle: '.dragHandle', update: function() {
        ...
        $.post('animatics_crud.php', {action:'reorder', order:o}, function(){ Toast.show('Saved','success'); });
    }});
}
</script>
