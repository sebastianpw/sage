<?php
// public/rapid_lore_api_processor.php
// "The Curator's Studio" - Generates Showcases from Curated Database Entities
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($input['action'] === 'load_doc_entities') {
        try {
            ...
            $service = new LoreAccessService($spw->getPdo());
            $service->loadDoc($docId);
            
            // Get Story Level Items (Episodes, Hooks, Engine)
            $storyData = $service->getStoryEngine();

            $payload =[
                // Story Elements (Fetched from Story Engine)
                'episodes' => $storyData['episodes'] ??[],
                'scene_hooks' => $storyData['scene_hooks'] ?? [],
                'narrative_engine' => $storyData['narrative_engine'] ??[],
                
                // World Elements (Fetched from Entity Index)
                'characters' => $service->queryEntities('characters'),
                'locations' => $service->queryEntities('locations'),
                'factions' => $service->queryEntities('factions'),
                'artifacts' => $service->queryEntities('artifacts')
            ];
    ...
    if ($input['action'] === 'fetch_context') {
        try {
            ...
            // 1. Semantic Data
            $contextStr = "";
            $contextData = $service->buildAgentContext($entityName);
            
            if ($contextData) {
                $contextStr = "IDENTITY:\n" . json_encode($contextData['identity'], JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE) . "\n\n";
                $contextStr .= "RELATIONSHIPS:\n" . implode("\n", $contextData['network']) . "\n\n";
                $contextStr .= "TIMELINE/HISTORY:\n" . implode("\n", $contextData['history']);
            } else {
                $story = $service->getStoryEngine();
                $foundItem = null;
                $foundType = 'Story Element';

                foreach ($story as $key => $items) {
                    if (is_array($items)) {
                        foreach ($items as $item) {
                            $itemName = $item['name'] ?? ($item['title'] ?? ($item['event'] ?? ''));
                            if (strcasecmp(trim($itemName), trim($entityName)) === 0) {
                                $foundItem = $item;
                                $foundType = ucfirst(str_replace('_', ' ', $key));
                                break 2;
                            }
                        }
                    }
                ...
                if ($foundItem) {
                    $contextStr = "TYPE: {$foundType}\nNAME: {$entityName}\n\nDETAILS:\n" . json_encode($foundItem, JSON_PRETTY_PRINT|JSON_UNESCAPED_UNICODE);
                } else {
                    $contextStr = "No semantic data found for this entity.";
                }
            }

            // 2. Visual Sketch Lookup
            $sketchData = null;
            if ($docId && $entityName && $entityType) {
                // Find latest sketch generated for this entity
                $sqlHistory = "
                ...
                ";
                $historyRow = $conn->fetchAssociative($sqlHistory,[$docId, $entityType, $entityName]);
            ...
            // 3. Mini-Graph References Lookup
            $agNodeId = 0;
            $kgNodeId = 0;
            if ($docId && $entityName) {
                try {
                    $agNodeId = (int)$conn->fetchOne("SELECT id FROM ag_nodes WHERE doc_id = ? AND LOWER(name) = LOWER(?) AND status='active' LIMIT 1", [$docId, $entityName]);
                } catch (\Exception $e) {}
            }
            if ($entityName) {
                try {
                    $kgNodeId = (int)$conn->fetchOne("SELECT id FROM kg_nodes WHERE LOWER(name) = LOWER(?) AND status='active' LIMIT 1", [$entityName]);
                } catch (\Exception $e) {}
            }
    ...
    if ($input['action'] === 'publish_showcase') {
        try {
            ...
            $categoryName = sanitizeText($input['category_name'] ?? 'API Imported');
...
$docs = $conn->fetchAllAssociative("
    SELECT d.id, d.name, c.name as cat_name 
...
<head>
    ...
    <title>Lore API Processor</title>
    ...
    <style>
        ...
        .swiper-slide { width: auto; height: 100%; display: flex; align-items: center; justify-content: center; background: #000; border-radius: 4px; overflow: hidden; border: 1px solid #333; position: relative; }
...
<div class="nav-toggle-btn" onclick="toggleSidebar()" title="Browse Lore Entities">
...
<div class="layout">
    ...
    <div class="flyout-sidebar" id="flyoutSidebar">
        <div class="sidebar-header">
            <h3 style="margin:0 0 10px 0; color:#fff;">🔮 Lore API Access</h3>
            <div class="doc-select-wrap">
                <select id="docSelect" class="doc-select" onchange="loadEntities()">
                    <option value="">-- Select Curated Doc --</option>
        ...
        <div class="sidebar-list" id="entityList">
            <div style="padding:20px; text-align:center; color:#555;">Select a document to load entities.</div>
        </div>
    </div>
    ...
    <div class="main">
            ...
            <div class="editor-box">
                ...
                <div class="col">
                    ...
                    <div style="flex:1; display:flex; flex-direction:column; min-height:0;">
                        <h3 style="margin:0;">Semantic Data Context</h3>
                    ...
                    <div id="visualContainer" class="visual-container">
                        ...
                        <!-- Added hardcoded overflow:hidden and max-width:100% to ensure visual bounds are strictly enforced -->
                        <div class="swiper pswp-gallery" id="sketchSwiper" style="width:100%; max-width:100%; height:240px; margin-bottom:10px; overflow:hidden;">
                ...
                <div class="col" id="colRight">
                    <h3 style="margin:0;">Generated Showcase Markdown</h3>
        ...
        <div id="emptyState" style="text-align:center; padding-top:100px; color:#555;">
            <h2>Select a curated entity from the sidebar</h2>
        </div>
    </div>
</div>
...
function loadEntities() {
    ...
    currentDocName = $('#docSelect option:selected').text();
    if(!docId) return;
    currentDocId = docId;
...
function renderList(data) {
    let html = '';
    const types =['episodes', 'scene_hooks', 'characters', 'locations', 'factions', 'artifacts'];
    ...
    types.forEach(type => {
        if (data[type] && data[type].length > 0) {
            ...
            const isMsgOpen = localStorage.getItem('lore_cat_' + type) === 'open';
            ...
            data[type].forEach(item => {
                const name = item.name || item.title || item.event || 'Unknown';
                // Use JS escapes for double quotes and HTML entities for single quotes to pass through safely
                const safeName = name.replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/'/g, '&#39;');
                const meta = `{ "name": "${safeName}", "type": "${type}" }`;
                const displayName = name.replace(/</g, '&lt;').replace(/>/g, '&gt;');
                html += `<div class="entity-row" onclick='selectEntity(this, ${meta})'><span>${displayName}</span></div>`;
            });
...
function toggleCat(type) {
    ...
    if (isHidden) {
        ...
        localStorage.setItem('lore_cat_' + type, 'open');
    } else {
        group.slideUp(200);
        icon.css('transform', 'rotate(0deg)');
        localStorage.setItem('lore_cat_' + type, 'closed');
    }
}
...
function selectEntity(el, meta) {
    ...
    $('#wkType').text(meta.type.replace('_', ' '));
    $('#btnPublish').hide();
    $('#btnAddScript').hide();
    $('#mdOutput').val('');
    
    $('#rawContent').val('Loading context from Lore API...');
    ...
    // Fetch Data
    $.post('', { action: 'fetch_context', doc_id: currentDocId, entity_name: meta.name, entity_type: meta.type }, function(res) {
        if(res.ok) {
...
function renderVisuals(sketch) {
    ...
    $('#sketchDesc').val(sketch.description);
    ...
    if (sketch.frames && sketch.frames.length > 0) {
        ...
        // Must include swiper-slide class so Swiper calculations don't break if no images exist
...
// --- GENERATION ---
function runAI() {
    if(!currentEntity || !currentDocId) return;
...
// --- NEW SCRIPT LIBRARY INTEGRATION ---
