...
<head>
  ...
  <style>
    body { font-family: Arial, sans-serif; background: #f8f9fa; padding: 20px; line-height: 1.6; color: #222; }
    h1 { margin-bottom: 12px; }

    ul#tree, ul#tree ul { list-style-type: none; padding-left: 16px; margin: 6px 0; }
    #tree > li {
      margin: 8px 0;
      padding: 8px 48px 8px 44px; /* room for toggle on left + handle on right */
      border-radius: 6px;
      background: #fff;
...
<body>
  ...
  $stmt1 = $pdo->prepare("SELECT id, name, href, position FROM pages_dashboard WHERE level = 1 ORDER BY position ASC, id ASC");
  $stmt1->execute();
  $tops = $stmt1->fetchAll(PDO::FETCH_ASSOC);

  $childStmt = $pdo->prepare("SELECT id, name, href, position, parent_id FROM pages_dashboard WHERE level = 2 AND parent_id = :pid ORDER BY position ASC, id ASC");
  ?>
