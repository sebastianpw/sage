<?php
// public/auto_narratives.php
// Showrunner V9 - Auto-Narrative Laboratory
// ----------------------------------------------------
// V5 UI Updates:
...
<div class="lab-layout">
    ...
    <div class="lab-header">
        ...
        <a href="/narratives.php" class="res-btn" style="padding: 6px 12px; text-decoration: none; display: inline-block;">Exit to Editor</a>
    </div>
    ...
    <div class="lab-content">
        ...
        <div class="left-stack">
            ...
            <div id="monitorTerm" style="max-height:360px;" class="monitor-panel">
                <div class="log-line">> Showrunner Auto-Narrative System v1.5</div>
...
function runGenerationCycle() {
    ...
    $.post('auto_narratives_api.php', payload, function(data) {
...
function applyAdvancedFilter() {
    ...
    currentFilter = { text: freeText, items: items };
    const count   = items.length;
    if (count > 0 || freeText) {
        $('#activeFilterBadge').show().text(`Active: ${count} entities + text`);
...
function playbackLogs(logs, callback) {
    ...
    const interval = setInterval(() => {
        ...
        else if (/^Vector/i.test(line) || /Chroma/i.test(line))   type = 'vector';
        else if (/^Error|^Warning|broken/i.test(line))            type = 'warn';
        else if (/^Generated|^Seed|^Selected/i.test(line))        type = 'success';
        else if (/^Director|^AI Director|^Lore|^World/i.test(line)) type = 'info';
...
function refreshResultList() {
    $.get('auto_narratives_api.php?action=list_results', function(res) {
...
function promoteSequence(id) {
    $.post('auto_narratives_api.php', { action: 'promote', id: id }, function(d) {
...
function deleteSequence(id) {
    if (confirm('Delete this sequence?')) {
        $.post('auto_narratives_api.php', { action: 'delete', id: id }, refreshResultList, 'json');
    }
}
</script>
<?php
