<?php
// public/cli_md_curator_extract_json.php
// PART 1: EXTRACTOR (Crawler) - JSON-capable
// - Preserves the original extraction logic
// - Adds JSON config support
// - Falls back to original interactive category selection when no JSON config is provided
//
// Usage:
//   Interactive: php cli_md_curator_extract_json.php
//   CLI args:    php cli_md_curator_extract_json.php --cat=3 --limit=10 --chars=4000 --model=gpt-4o-mini
//   JSON file:   php cli_md_curator_extract_json.php --config=md_curator_job.json
//   JSON inline: php cli_md_curator_extract_json.php --config-json='{"targetCategoryId":3,"limit":10,"charLimit":4000,"overrideModel":"gpt-4o-mini"}'
// ----------------------------------------------------
...
// Parse CLI args and optional JSON config
$opts = getopt('', [
    'cat::',
    'limit::',
    'chars::',
...
if (isset($opts['config'])) {
    $jsonMode = true;
    $cfg = json_decode(file_get_contents($opts['config']), true);
    if (!is_array($cfg)) {
        die(C_RED . "Error: Invalid JSON config file.\n" . C_RESET);
    }
    ...
    // Optional aliases, harmless if present
...
} elseif (isset($opts['config-json']) && trim((string)$opts['config-json']) !== '') {
    $jsonMode = true;
    $cfg = json_decode($opts['config-json'], true);
    if (!is_array($cfg)) {
        die(C_RED . "Error: Invalid JSON passed to --config-json.\n" . C_RESET);
    }
    $targetCategoryId  = array_key_exists('targetCategoryId', $cfg) ? (int)$cfg['targetCategoryId'] : 0;
    $limit             = array_key_exists('limit', $cfg) ? (int)$cfg['limit'] : $limit;
    $charLimit         = array_key_exists('charLimit', $cfg) ? (int)$cfg['charLimit'] : $charLimit;
