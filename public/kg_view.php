...
/* ── Fuzz pill on drawer handle ── */
.kg-fuzz-pill {
    display: none;              /* shown via JS when a candidate exists */
    align-items: center;
    gap: 5px;
    position: absolute;
    right: 56px;                /* clear of the hamburger area */
    top: 50%;
...
.kg-fuzz-pill i { font-size: 0.85rem; }

/* ── Gallery pill on drawer handle ── */
.kg-gallery-pill {
    display: none;              /* shown via JS when visuals exist */
    align-items: center;
    gap: 5px;
    position: absolute;
    top: 50%;
...
.kg-gallery-pill:hover {
    background: rgba(6, 182, 212, 0.22);
    border-color: #06b6d4;
}
.kg-gallery-pill i { font-size: 0.85rem; }
...
.kg-gallery-modal-bg.open { display: flex; }

.kg-gallery-modal-header {
    height: 52px;
    min-height: 52px;
    background: var(--card);
    border-bottom: 1px solid var(--border);
...
.kg-gallery-graph-pills {
    display: flex;
    align-items: center;
    gap: 6px;
    flex-shrink: 0;
}
.kg-gallery-graph-pill {
...
.kg-gallery-graph-pill:hover { background: rgba(59,130,246,0.20); }

.kg-gallery-modal-close {
    background: none;
    border: 1px solid var(--border);
    color: var(--text-muted);
    border-radius: 6px;
...
.kg-gallery-modal-close:hover { color: var(--text); background: var(--bg); }

.kg-gallery-modal-body {
    flex: 1;
    display: flex;
    flex-direction: column;
    align-items: center;
...
/* Fullscreen swiper fills available width */
#kgGallerySwiper {
    width: 100%;
...
#kgGallerySwiper .swiper-slide img {
    width: 100%;
    height: 100%;
    object-fit: contain;
    display: block;
}
#kgGallerySwiper .swiper-button-next,
#kgGallerySwiper .swiper-button-prev {
    color: #fff;
    text-shadow: 0 2px 6px rgba(0,0,0,0.8);
}

/* Detail button on each slide */
.kg-gal-detail-btn {
    position: absolute;
    top: 8px; right: 8px;
...
#kgGallerySwiper .swiper-slide:hover .kg-gal-detail-btn { opacity: 1; }
.kg-gal-detail-btn:hover { background: rgba(255,255,255,0.15); }
...
.kg-frame-view-modal.active { display: flex; }
.kg-frame-view-content {
...
.kg-frame-view-close:hover { background: #fff; color: #000; }
iframe.kg-frame-viewer { width: 100%; height: 100%; border: none; }

/* ── Mini-graph iframe modal ── */
.kg-minigraph-modal-bg {
    position: fixed; inset: 0;
...
<button id="kg-export-btn" onclick="openExportModal()" title="Export JSON Snapshot"
...
<div class="kg-layout">
    ...
    <div class="kg-topbar">
        ...
        <div class="kg-topbar-right">
            ...
            <!-- ── Link to Graph Visualizer ── -->
            <a href="kg_graph.php" class="btn btn-ghost btn-sm" title="Open Graph Visualizer">
...
<div class="kg-bottom-drawer" id="kg-bottom-drawer">
    <div class="kg-drawer-handle" onclick="toggleBottomDrawer()">
        ...
        <a id="kg-fuzz-pill"
           ...
           target="_blank"
           rel="noopener"
           title="Open promoted Fuzz candidate"
           onclick="event.stopPropagation();">
            <i class="bi bi-diagram-3-fill"></i> FUZZ
        </a>
        <!-- ── Gallery pill ── -->
        <button id="kg-gallery-pill"
                class="kg-gallery-pill"
...
<!-- ═══════ FULLSCREEN GALLERY MODAL ═══════ -->
<div class="kg-gallery-modal-bg" id="kg-gallery-modal-bg">
    <div class="kg-gallery-modal-header">
        <span class="kg-gallery-modal-title" id="kg-gallery-modal-title">Visual Gallery</span>
        <!-- ── Mini-graph pills ── -->
        <div class="kg-gallery-graph-pills" id="kg-gallery-graph-pills"></div>
        <button class="kg-gallery-modal-close" onclick="closeGalleryModal()" title="Close">&#x2715;</button>
    </div>
    <div class="kg-gallery-modal-body">
        <div class="swiper" id="kgGallerySwiper">
            <div class="swiper-wrapper" id="kgGallerySwiperWrapper"></div>
            <div class="swiper-button-next"></div>
            <div class="swiper-button-prev"></div>
        </div>
        <textarea class="kg-gallery-desc" id="kgGalleryDesc" readonly placeholder="Sketch description…"></textarea>
    </div>
</div>

<!-- ═══════ FRAME DETAIL MODAL (triggered from gallery) ═══════ -->
<div class="kg-frame-view-modal" id="kg-frame-view-modal">
    <div class="kg-frame-view-content">
        <div class="kg-frame-view-close" onclick="closeKgFrameModal()"><i class="bi bi-x-lg"></i></div>
        <iframe id="kg-frame-viewer" class="kg-frame-viewer" src=""></iframe>
    </div>
</div>

<!-- ═══════ MINI-GRAPH IFRAME MODAL ═══════ -->
<div class="kg-minigraph-modal-bg" id="kg-minigraph-modal-bg">
    <div class="kg-minigraph-inner">
        <button class="kg-minigraph-close" onclick="closeKgMiniGraph()" title="Close">&#x2715;</button>
        <iframe class="kg-minigraph-iframe" id="kg-minigraph-iframe" src=""></iframe>
    </div>
</div>
...
<div class="kge-modal-bg" id="kge-modal-bg">
    <div class="kge-modal">
        ...
        <div class="kge-tabs">
            <button class="kge-tab active" id="kge-tab-full"     onclick="kgeSetTab('full')">Full Graph</button>
        ...
        <div class="kge-panes">
            ...
            <!-- ══ Full Export Pane ══ -->
            <div class="kge-pane active" id="kge-pane-full">
                    ...
                    <div class="kge-options-strip">
                        ...
                        <div class="kge-option-row">
                            ...
                            <label for="kge-full-with-edges">
                                ...
                                <span style="color:var(--text-muted); font-size:0.78rem; display:block; margin-top:1px;">Exports graph relationships (kg_node_items).</span>
                            </label>
                        </div>
                    </div>
                </div>
...
// DRAG & DROP TOGGLE
// ═══════════════════════════════════════════════
const KG_DND_KEY = 'kg_dnd_enabled';
...
function initTree() {
    ...
    $('#kg-tree').jstree({
        core: {
            data: {
                url: 'kg_api.php?action=fetch_tree',
                dataType: 'json',
                dataFilter: raw => {
    ...
    .on('move_node.jstree', (e, data) => {
        $.post('kg_api.php', { action:'move_node', id:data.node.id, parent:data.parent })
    ...
    if (node.type === 'folder') {
        items.rename = {
            ...
            action: () => {
                ...
                if (name && name.trim()) {
                    $.post('kg_api.php', {action:'rename_category', id:node.data.db_id, name:name.trim()}, res => {
        ...
        items.del = {
            ...
            action: () => {
                if (confirm('Delete folder? Nodes inside will move to root.')) {
                    $.post('kg_api.php', {action:'delete_category', id:node.data.db_id}, res => {
                        if (res.ok) refreshTree(); else toast('Error','error');
                    }, 'json');
                }
            }
        ...
        items.rename = {
            ...
            action: () => {
                ...
                if (name && name.trim()) {
                    $.post('kg_api.php', {action:'rename_node', id:node.data.db_id, name:name.trim()}, res => {
        ...
        items.del = {
            ...
            action: () => {
                if (confirm('Archive this node?')) {
                    $.post('kg_api.php', {action:'delete_node', id:node.data.db_id}, res => {
...
function loadCategories() {
    $.get('kg_api.php?action=fetch_tree', res => {
        if (!res.ok) return;
...
function loadNode(id) {
    ...
    currentNodeId = id;
    $.get('kg_api.php?action=get_node&id=' + id, res => {
        ...
        // ── Show FUZZ pill if a promoted candidate is linked ──
        const fuzzPill = document.getElementById('kg-fuzz-pill');
        fuzzPill.style.display = 'none';
        fuzzPill.href = '#';
        fetch('kg_api.php?action=get_fuzz_candidate_for_node&node_id=' + id)
            .then(r => r.json())
            .then(res => {
                if (res.ok && res.candidate) {
                    fuzzPill.href = 'fuzz_forge_landing.php?id=' + res.candidate.id;
                    fuzzPill.style.display = 'inline-flex';
                }
                repositionGalleryPill();
            })
            .catch(() => { repositionGalleryPill(); });

        // ── Fetch visuals and show/hide gallery pill ──
...
function showEmptyState() {
    ...
    document.getElementById('kg-bottom-drawer').classList.remove('open');
    document.getElementById('kg-gallery-pill').style.display = 'none';
}

// ═══════════════════════════════════════════════
// GALLERY — fetch visuals, store, open modal
// ═══════════════════════════════════════════════
let kgCurrentGalleryData = null;
let kgGallerySwiper      = null;

function repositionGalleryPill() {
    const pill     = document.getElementById('kg-gallery-pill');
    const fuzzPill = document.getElementById('kg-fuzz-pill');
    if (pill.style.display === 'none') return;
    const fuzzVisible = (fuzzPill.style.display !== 'none' && fuzzPill.style.display !== '');
    pill.style.right = fuzzVisible ? '146px' : '56px';
}

function fetchNodeVisuals(nodeName) {
    kgCurrentGalleryData = null;
    const pill = document.getElementById('kg-gallery-pill');
    pill.style.display = 'none';
    // Clear graph pills from previous node
    document.getElementById('kg-gallery-graph-pills').innerHTML = '';

    $.post('kg_api.php', { action: 'fetch_visuals', entity_name: nodeName }, function(res) {
        if (res.ok && res.sketch && res.sketch.frames && res.sketch.frames.length > 0) {
            kgCurrentGalleryData = res;
            repositionGalleryPill();
            pill.style.display = 'inline-flex';

            // Pre-build mini-graph pills for the gallery header
            let pillsHtml = '';
            
            const kgNodeId = currentNodeId;
            
            if (kgNodeId) {
                const kgUrl = `mini_graph.php?graph=kg&node_id=${kgNodeId}`;
                pillsHtml += `<a class="kg-gallery-graph-pill" href="${kgUrl}" target="_blank">🔮 KG</a>
                              <button class="kg-gallery-graph-pill" onclick="openKgMiniGraph('${escHtmlAttr(kgUrl)}')">⤢ modal</button>`;
            }
            if (res.ag_node_id > 0) {
                const agUrl = `mini_graph.php?graph=ag&doc_id=${res.ag_doc_id || 0}&node_id=${res.ag_node_id}`;
                if (pillsHtml) pillsHtml += '<span style="color:var(--border); margin:0 2px;">|</span>';
                pillsHtml += `<a class="kg-gallery-graph-pill" href="${agUrl}" target="_blank">📜 AG</a>
                              <button class="kg-gallery-graph-pill" onclick="openKgMiniGraph('${escHtmlAttr(agUrl)}')">⤢ modal</button>`;
            }
            document.getElementById('kg-gallery-graph-pills').innerHTML = pillsHtml;
        }
    }, 'json').fail(() => {});
}

function openGalleryModal() {
    if (!kgCurrentGalleryData || !kgCurrentGalleryData.sketch) return;

    const sketch   = kgCurrentGalleryData.sketch;
    const nodeName = document.getElementById('kg-node-title').textContent || 'Visual Gallery';

    document.getElementById('kg-gallery-modal-title').textContent =
        '🖼️ ' + (sketch.name || nodeName);
    document.getElementById('kgGalleryDesc').value = sketch.description || '';

    // Build slides
    const wrapper = document.getElementById('kgGallerySwiperWrapper');
    wrapper.innerHTML = '';
    sketch.frames.forEach(f => {
        ...
        slide.innerHTML = `
            ...
            <button class="kg-gal-detail-btn"
                    onclick="event.stopPropagation(); openKgFrameModal(${parseInt(f.id)})"
    ...
    document.getElementById('kg-gallery-modal-bg').classList.add('open');

    if (kgGallerySwiper) { kgGallerySwiper.destroy(true, true); kgGallerySwiper = null; }
    setTimeout(() => {
        kgGallerySwiper = new Swiper('#kgGallerySwiper', {
            slidesPerView: 1,
            spaceBetween: 0,
            navigation: {
                nextEl: '#kgGallerySwiper .swiper-button-next',
                prevEl: '#kgGallerySwiper .swiper-button-prev',
            },
        });
    }, 50);
}
...
function closeGalleryModal() {
    document.getElementById('kg-gallery-modal-bg').classList.remove('open');
    if (kgGallerySwiper) { kgGallerySwiper.destroy(true, true); kgGallerySwiper = null; }
}

function openKgFrameModal(frameId) {
    document.getElementById('kg-frame-viewer').src = `view_frame.php?frame_id=${frameId}&view=modal`;
    document.getElementById('kg-frame-view-modal').classList.add('active');
}

function closeKgFrameModal() {
    document.getElementById('kg-frame-view-modal').classList.remove('active');
    setTimeout(() => { document.getElementById('kg-frame-viewer').src = ''; }, 200);
}

// ═══════════════════════════════════════════════
// MINI-GRAPH MODAL
// ═══════════════════════════════════════════════
function openKgMiniGraph(url) {
    document.getElementById('kg-minigraph-iframe').src = url;
    document.getElementById('kg-minigraph-modal-bg').classList.add('open');
}

function closeKgMiniGraph() {
    document.getElementById('kg-minigraph-modal-bg').classList.remove('open');
    setTimeout(() => { document.getElementById('kg-minigraph-iframe').src = ''; }, 200);
}
...
function saveNode() {
    ...
    $.post('kg_api.php', {action:'save_node', ...payload}, res => {
...
function deleteCurrentNode() {
    ...
    $.post('kg_api.php', {action:'delete_node', id:currentNodeId}, res => {
...
function renderLinkedItems(items) {
    ...
    list.innerHTML = items.map(item => {
        ...
        if (extUrl) {
            ...
            extBtn = `
                <a class="kg-item-ext-btn"
                   href="kg_view.php?node_id=${extId}" target="_blank"
                   title="Open in new tab">
                    <i class="bi bi-box-arrow-up-right"></i>
                </a>`;
        }
...
function removeLinkedItem(id) {
    $.post('kg_api.php', {action:'remove_item', id}, res => {
...
function createFolder() {
    ...
    if (!name) return;
    $.post('kg_api.php', {action:'create_category', name, parent_id:parentId}, res => {
...
function createNode() {
    ...
    if (!name) return;
    $.post('kg_api.php', {action:'create_node', name, node_type:nodeType, category_id:catId}, res => {
        if (res.ok) {
            hideModal('modalNode');
...
document.getElementById('kg-gallery-modal-bg').addEventListener('click', function(e) {
    if (e.target === this) closeGalleryModal();
});
document.getElementById('kg-minigraph-modal-bg').addEventListener('click', function(e) {
    if (e.target === this) closeKgMiniGraph();
});
...
function saveEditFolder() {
    ...
    if (!id || !name) return;
    $.post('kg_api.php', {action:'rename_category', id, name}, res => {
...
// ══════════════════════════════════════════════
// PICKER TREE — load, render, selection
// ══════════════════════════════════════════════
...
async function kgeDoFullExport() {
    ...
    try {
        if (nodeIds === null && !withContent) {
            const res  = await fetch(`kg_api.php?action=export_snapshot&with_edges=${withEdges ? 1 : 0}`);
            const data = await res.json();
            if (!data.ok) throw new Error(data.error || 'Export failed');
            const snap = data.snapshot;
            if (!withEdges) snap.edges = { fields: snap.edges.fields, rows: [] };
            kgeTriggerDownload(snap, 'kg_full_' + kgeDate() + '.json');
            ...
            kgeTriggerDownload(data.snapshot, `kg_export${suffix}_${kgeDate()}.json`);
...
async function kgeDoFocusedExport() {
    ...
    try {
        ...
        if (!data.ok) throw new Error(data.error || 'Snapshot failed');
        const suffix = withContent ? '_content' : '';
        kgeTriggerDownload(data.snapshot, `kg_semantic${suffix}_${kgeDate()}.json`);
...
function importMdDocEntities() { window.location.href = 'view_kg_import.php'; }
