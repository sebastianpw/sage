<?php
// public/view_scrollmagic_rating.php
...
$pageTitle = "Sage Rating Viewer";

// --- GET Params ---
$entity_type = $_GET['entity_type'] ?? '';
$entity_id = $_GET['entity_id'] ?? '';
$query = $_GET['query'] ?? '';
$sort_by = $_GET['sort_by'] ?? 'frames.created_at';
...
<style>
    ...
    .topbar-controls { display: flex; gap: 6px; flex-wrap: wrap; align-items: center; flex: 1; }
    ...
    /* MODIFIED: Compact Widths */
    .ms-container { flex: 0 1 160px; min-width: 100px; max-width: 160px; }
    ...
    /* MODIFIED: Compact Widths */
    .logic-select { flex: 0 1 160px; min-width: 100px; max-width: 160px; background: #1a1a1a; color: #ccc; border: 1px solid #444; border-radius: 4px; height: 36px; padding: 0 8px; font-size: 13px; }
    ...
    @media (max-width: 768px) {
        ...
        /* On mobile, let them grow a bit, but keep strict flow */
        .ms-container { flex: 1 1 45%; max-width: none; }
        .logic-select { flex: 1 1 45%; max-width: none; }
        .rating-filter { order: 3; flex: 1 1 auto; justify-content: center; }
        #apply-storyboard-filter { order: 4; }
        /* Ensure input stays near button */
...
<div class="topbar" id="main-topbar">
    ...
    <div class="topbar-controls">
        ...
        <select id="assignment-logic" class="logic-select" title="Filtering Logic">
            <option value="" <?= $pre_logic === '' ? 'selected' : '' ?>>(No Filter)</option>
            <option value="global_any" <?= $pre_logic === 'global_any' ? 'selected' : '' ?>>Global Any</option>
            <option value="selected_any" <?= $pre_logic === 'selected_any' ? 'selected' : '' ?>>Any (Selected)</option>
            <option value="selected_all" <?= $pre_logic === 'selected_all' ? 'selected' : '' ?>>All (Selected)</option>
            <option value="selected_multi" <?= $pre_logic === 'selected_multi' ? 'selected' : '' ?>>Multi (Selected)</option>
            <option value="global_multi" <?= $pre_logic === 'global_multi' ? 'selected' : '' ?>>Global Multi</option>
        </select>
        
        <!-- Star Rating Filter -->
        <div id="star-filter" class="rating-filter" title="Filter by Rating (Exact)">
        ...
        <!-- Compact Frame Counter & Jump Input -->
...
<div class="view-container" id="view-container">
    ...
    <div id="gallery-config" style="display:none"
         data-api-url="/scrollmagic_images_rating.php"
...
(function(){
    ...
    const els = {
        ...
        starContainer: document.getElementById('star-filter'),
    ...
    // --- Star Rating Logic ---
    const starEls = els.starContainer.querySelectorAll('.rating-star');
    
    function renderStars() {
        starEls.forEach(star => {
            const val = parseInt(star.dataset.val);
            if (val <= API_CONFIG.rating) {
    ...
    starEls.forEach(star => {
        star.addEventListener('click', () => {
            const val = parseInt(star.dataset.val);
            if (API_CONFIG.rating === val) {
                API_CONFIG.rating = 0;
            } else {
                API_CONFIG.rating = val;
            }
            renderStars();
        });
    });
    
    //API_CONFIG.rating = 5; renderStars();
    // --------------------------

    const getStorageKey = () => `sage_rating_pos_${API_CONFIG.entityType||'all'}_${API_CONFIG.assignmentLogic}_${API_CONFIG.rating}`;
    ...
    async function fetchApi(offset, limit) {
        ...
        // Pass rating
        if(API_CONFIG.rating > 0) p.set('rating', API_CONFIG.rating);
