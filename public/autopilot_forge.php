<?php
// public/autopilot_forge.php
// Mobile-first JSON builder for Sketch Autopilot
// Writes config for:
//   php public/cli_autopilot_json.php --config=job.json
...
function loadRapidGenerators($conn): array {
    $sql = "
...
function buildDefaults(array $ingredientsConfig, array $generatorList): array {
    $defaults = [];
    ...
    return [
        'desc_gen_id' => $generatorList[0]['id'] ?? 0,
        'amount' => 0,
...
function getIngredientMeta(array $ingredientsConfig): array {
    $out = [];
    foreach ($ingredientsConfig as $key => $conf) {
        $out[] = [
            'key' => $key,
...
if (isset($_GET['action']) && $_GET['action'] === 'meta') {
    ...
    $gens = loadRapidGenerators($conn);
    $ingredients = getIngredientMeta($ingredientsConfig);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_json') {
    ...
    $dir = __DIR__ . '/../var/autopilot_configs';
    if (!is_dir($dir)) {
        @mkdir($dir, 0775, true);
    }

    $name = preg_replace('/[^a-zA-Z0-9_\-]+/', '_', trim((string)($_POST['name'] ?? 'autopilot_job')));
    ...
    echo json_encode([
        ...
        'saved' => true,
        'path' => $file,
        'runner' => 'php public/cli_autopilot_json.php --config=' . $file,
    ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    exit;
}

$generators = loadRapidGenerators($conn);
$defaults = buildDefaults($ingredientsConfig, $generators);
$ingredientMeta = getIngredientMeta($ingredientsConfig);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<main class="main">
    <div class="shell">
        ...
        <div class="card desktop-span">
            ...
            <div class="card-body">
                <div class="help">
                    Build a JSON preset here, then run it with the CLI runner. Amount 0 means infinite.
                </div>
            </div>
        </div>
        ...
        <details class="section" open>
            <summary class="section-head">
                <div class="section-head-left">
                    ...
                    <div class="section-label">
                        <strong>Generator and Run Count</strong>
                        <span>Select the rapidcreate generator and number of sketches.</span>
                    </div>
                </div>
            ...
            <div class="section-content">
                <div class="field">
                    <div class="label">Description Generator</div>
                    <select id="desc_gen_id"></select>
                </div>

                <div class="field">
                    <div class="label">Amount</div>
                    <input type="number" id="amount" min="0" step="1" value="0">
                    <div class="help">Use 0 for infinite mode.</div>
                </div>
            </div>
        </details>
        ...
        <details class="section" open>
            <summary class="section-head">
                <div class="section-head-left">
                    ...
                    <div class="section-label">
                        <strong>Ingredient Probabilities</strong>
                        <span>Set the chance for each ingredient family.</span>
                    </div>
                </div>
                <div class="section-chevron"><i class="bi bi-chevron-down"></i></div>
            </summary>
            <div class="section-content">
                <div class="ingredient-grid" id="probSection">
                    <!-- populated by JS -->
                </div>
            </div>
        </details>
            ...
            <summary class="section-head">
                <div class="section-head-left">
                    ...
                    <div class="section-label">
                        ...
                        <span>Generator list and defaults update from the server.</span>
                    </div>
                </div>
                <div class="section-chevron"><i class="bi bi-chevron-down"></i></div>
            </summary>
            <div class="section-content">
                <div class="stats-grid">
                    ...
                    <div class="stat">
                        ...
                        <div class="stat-meta">
                            <div class="command" id="cmdOut">php public/cli_autopilot_json.php --config=&lt;file.json&gt;</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="stat">
                    <div class="stat-top">
                        <div class="stat-name">Generators</div>
                        <div class="stat-badge" id="genCountBadge">—</div>
                    </div>
                    <div class="stat-meta">
                        <div id="genStats" class="stats-grid"></div>
                    </div>
                </div>
            </div>
        </details>
...
function getPayload() {
    const probs = {};
    for (const item of ingredientMeta) {
        const el = document.getElementById(`prob_${item.key}`);
        const num = parseInt(el?.value || String(item.default || 0), 10);
        probs[item.key] = Math.max(0, Math.min(100, num));
    }

    return {
        desc_gen_id: parseInt(document.getElementById('desc_gen_id').value || '0', 10),
...
function updateJson() {
    ...
    const genText = document.getElementById('desc_gen_id').selectedOptions[0]?.text || '';
    document.getElementById('scopeBadge').textContent = payload.amount === 0 ? 'Infinite' : String(payload.amount);
    document.getElementById('scopeSummaryShort').textContent = payload.amount === 0 ? 'Infinite' : `${payload.amount} runs`;

    const probLine = ingredientMeta
        .map(i => `${i.key}=${payload.probabilities[i.key]}%`)
        .join(' · ');

    document.getElementById('scopeSummary').innerHTML =
        `<span class="ok">Generator:</span> ${esc(genText)}<br>` +
        `<span class="ok">Amount:</span> ${payload.amount === 0 ? 'Infinite' : payload.amount}<br>` +
        `<span class="ok">Mix:</span> ${esc(probLine || '—')}`;
}

function renderIngredientControls(items) {
    ingredientMeta = items || [];

    const box = document.getElementById('probSection');
    if (!ingredientMeta.length) {
        box.innerHTML = `<div class="stat"><div class="stat-meta">No ingredient definitions found.</div></div>`;
        return;
    }

    box.innerHTML = ingredientMeta.map(item => `
        <div class="ingredient-item">
            <div class="ingredient-top">
                ...
                <div class="ingredient-value" id="val_${esc(item.key)}">${esc(item.default)}%</div>
            </div>
            <input class="ingredient-slider" type="range" id="prob_${esc(item.key)}" min="0" max="100" step="1" value="${esc(item.default)}">
    ...
    for (const item of ingredientMeta) {
        const slider = document.getElementById(`prob_${item.key}`);
        const valueLabel = document.getElementById(`val_${item.key}`);
...
function fillGenerators(list) {
    generators = list || [];
    const select = document.getElementById('desc_gen_id');

    if (!generators.length) {
        select.innerHTML = `<option value="0">-- No generators found --</option>`;
        document.getElementById('genCountBadge').textContent = '0';
        document.getElementById('genStats').innerHTML = `<div class="stat"><div class="stat-meta">No generators tagged with rapidcreate.</div></div>`;
        return;
    }

    select.innerHTML = generators.map(g =>
        `<option value="${esc(g.id)}">${esc(g.title)} (${esc(g.model || 'openai')})</option>`
    ).join('');

    document.getElementById('genCountBadge').textContent = `${generators.length}`;
    document.getElementById('genStats').innerHTML = generators.map(g => `
        <div class="stat">
            <div class="stat-top">
                <div class="stat-name">${esc(g.title)}</div>
                <div class="stat-badge">#${esc(g.id)}</div>
            </div>
            <div class="stat-meta">${esc(g.model || 'openai')}</div>
        </div>
    `).join('');
}
...
async function loadMeta() {
    ...
    fillGenerators(data.generators || []);
    renderIngredientControls(data.ingredients || []);

    if (data.defaults) {
        if (data.defaults.desc_gen_id) {
            document.getElementById('desc_gen_id').value = String(data.defaults.desc_gen_id);
        }
        document.getElementById('amount').value = String(data.defaults.amount ?? 0);

        const probs = data.defaults.probabilities || {};
        for (const item of ingredientMeta) {
            const el = document.getElementById(`prob_${item.key}`);
            const label = document.getElementById(`val_${item.key}`);
            const val = (probs[item.key] !== undefined) ? probs[item.key] : item.default;
...
async function savePreset() {
    const name = prompt('Preset name:', 'autopilot_job');
    if (name === null) return;
...
function bindEvents() {
    document.getElementById('desc_gen_id').addEventListener('change', updateJson);
    document.getElementById('amount').addEventListener('input', updateJson);
