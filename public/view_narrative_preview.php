<?php
// public/view_narrative_preview.php
...
if (isset($_GET['api_action'])) {
    ...
    try {
        if ($action === 'list_sequences') {
            ...
            $total = (int)$pdo->query("SELECT COUNT(*) FROM narrative_sequences")->fetchColumn();
            $pages = (int)ceil($total / $limit) ?: 1;

            $stmt = $pdo->prepare(
                "SELECT id, name, description FROM narrative_sequences ORDER BY id DESC LIMIT $limit OFFSET $offset"
            );
            $stmt->execute();
            echo json_encode([
                'status' => 'ok',
        ...
        if ($action === 'zip_sequence_videos') {
            ...
            if (empty($rows)) throw new Exception('No videos found for this sequence');

            $publicPathAbs = rtrim($spw->getPublicPath(), '/');

            $nameStmt = $pdo->prepare("SELECT name FROM narrative_sequences WHERE id = ?");
            $nameStmt->execute([$seqId]);
            $seqName = $nameStmt->fetchColumn() ?: 'sequence_' . $seqId;
            $safeName = preg_replace('/[^a-z0-9_-]/i', '_', $seqName);
            $zipName  = 'seq_' . $seqId . '_' . $safeName . '.zip';
        ...
        if ($action === 'zip_download') {
            ...
            if (!preg_match('/^seq_\d+_[a-zA-Z0-9_\-]+\.zip$/', $zipName)) {
...
.np-seq-item .si-desc { font-size: 0.7rem; color: #5a5a8a; margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
<div class="np-layout">
    ...
    <div class="np-sidebar" id="npSidebar">
        <div class="np-sidebar-head">
            <span class="np-sidebar-title">Narrative Sequences</span>
        </div>
    ...
    <div class="np-main">
        ...
        <div class="np-topbar">
            <button class="np-hamburger" id="npHamburger" onclick="toggleSidebar()" title="Narrative Sequences">☰</button>
            <span class="np-seq-badge" id="npBadge" style="display:none;">SEQ</span>
            <span class="np-seq-name-label" id="npSeqLabel">Select a sequence →</span>
        ...
        <div class="np-player-wrap" id="npPlayerWrap">
            <div class="np-placeholder" id="npPlaceholder">
                ...
                <span>Select a sequence to begin</span>
            </div>
        ...
        <div class="np-controls">
            ...
            <button class="np-dl-btn" id="npDownload" disabled onclick="downloadSequenceZip()" title="Download all videos as ZIP">⬇</button>
...
async function loadSequences(page) {
    ...
    const res = await fetch(`?api_action=list_sequences&page=${page}&limit=${SEQ_PER_PAGE}`).then(r=>r.json());
    seqList.innerHTML = '';

    if (!res.status || res.status !== 'ok' || !res.data || !res.data.length) {
        seqList.innerHTML = '<div class="np-seq-empty">No sequences found.</div>';
        return;
    }
    ...
    sequences.forEach(seq => {
        ...
        el.className = 'np-seq-item' + (seq.id == curSeqId ? ' active' : '');
        el.dataset.id = seq.id;
        el.innerHTML = `
            <div class="si-id">#${seq.id}</div>
            <div class="si-name">${esc(seq.name)}</div>
            ${seq.description ? `<div class="si-desc">${esc(seq.description)}</div>` : ''}
        `;
        el.onclick = () => selectSequence(seq.id, seq.name);
        seqList.appendChild(el);
    });
}

// ── Select a sequence ──
async function selectSequence(seqId, seqName) {
    curSeqId   = seqId;
    curSeqName = seqName;
    curIdx     = -1;
    videos     =[];
    ...
    document.querySelectorAll('.np-seq-item').forEach(el => el.classList.toggle('active', el.dataset.id == seqId));

    badge.style.display   = 'inline-block';
    badge.textContent     = `#${seqId}`;
    ...
    const res = await fetch(`?api_action=get_sequence_videos&seq_id=${seqId}`).then(r=>r.json());
    gridState.style.display = 'none';

    if (!res.status || res.status !== 'ok' || !res.data || !res.data.length) {
        grid.innerHTML = '<div class="np-state" style="grid-column:1/-1;">No videos found for this sequence.</div>';
...
document.addEventListener('click', e => {
    ...
    if (btn.classList.contains('delete-video-btn')) {
        if (confirm('Delete video?')) {
            ...
            .then(r=>r.json()).then(d=>{
                if(d.status==='ok') { if(typeof Toast !== 'undefined') Toast.show('Deleted'); document.getElementById('videoDetailModal').classList.remove('active'); if (curSeqId) selectSequence(curSeqId, curSeqName); }
...
// ── Download ZIP ──
async function downloadSequenceZip() {
    if (!curSeqId || !videos.length) return;
    btnDl.classList.add('loading'); btnDl.textContent = '…';
    try {
        const res = await fetch(`?api_action=zip_sequence_videos&seq_id=${curSeqId}`).then(r => r.json());
...
window.downloadSequenceZip = downloadSequenceZip;
...
seqPrevBtn.addEventListener('click', () => loadSequences(seqPage - 1));
seqNextBtn.addEventListener('click', () => loadSequences(seqPage + 1));
seqPageInput.addEventListener('change', () => loadSequences(parseInt(seqPageInput.value)));
seqPageInput.addEventListener('keydown', e => { if (e.key === 'Enter') { loadSequences(parseInt(seqPageInput.value)); seqPageInput.blur(); } });
loadSequences(1);
