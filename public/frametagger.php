<?php
// public/frametagger.php
// Frames Tagger -- Mass Tagging Interface for Frames
// Cloned from Videos Tagger, adapted for Map Runs of Sketches/Frames
// ----------------------------------------------------
...
if (isset($_REQUEST['api_action'])) {
    ...
    try {
        ...
        if ($action === 'get_storyboards') {
            ...
            $whereParts = ["s.is_archived = 0"];
            $params = [];
            if ($search) {
                $whereParts[] = "(s.name LIKE ? OR s.custom_tag LIKE ? OR s.id = ?)";
                $params[] = "%$search%";
                $params[] = "%$search%";
            ...
            $countSql = "SELECT COUNT(s.id) FROM storyboards s WHERE $whereSQL";
            $stmtCount = $pdo->prepare($countSql);
            $stmtCount->execute($params);
            $total = $stmtCount->fetchColumn();

            $sql = "SELECT s.id, s.name as note, COUNT(sf.id) as item_count 
                    FROM storyboards s 
                    LEFT JOIN storyboard_frames sf ON s.id = sf.storyboard_id 
                    WHERE $whereSQL 
                    GROUP BY s.id 
                    ORDER BY s.updated_at DESC LIMIT $limit OFFSET $offset";
        ...
        if ($action === 'get_map_runs') {
            ...
            $whereParts =["m.entity_type = 'sketches'"];
            $params =[];
            if ($search) {
            ...
            $countSql = "SELECT COUNT(DISTINCT m.id) FROM map_runs m INNER JOIN frames f ON m.id = f.map_run_id WHERE $whereSQL";
            $stmtCount = $pdo->prepare($countSql);
            $stmtCount->execute($params);
            $total = $stmtCount->fetchColumn();

            $sql = "SELECT m.*, COUNT(f.id) as item_count FROM map_runs m INNER JOIN frames f ON m.id = f.map_run_id WHERE $whereSQL GROUP BY m.id ORDER BY m.id DESC LIMIT $limit OFFSET $offset";
            $stmt = $pdo->prepare($sql);
            $stmt->execute($params);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        if ($action === 'get_frames') {
            if (isset($_GET['storyboard_id'])) {
                $runId = (int)$_GET['storyboard_id'];
                $sql = "SELECT f.id, sf.name, f.filename as thumbnail, f.entity_id, f.entity_type 
                        FROM storyboard_frames sf 
                        JOIN frames f ON sf.frame_id = f.id 
                        WHERE sf.storyboard_id = $runId 
                        ORDER BY sf.sort_order ASC";
            } else {
                $runId = (int)$_GET['map_run_id'];
                $sql = "SELECT f.id, f.name, f.filename as thumbnail, f.entity_id, f.entity_type 
                        FROM frames f WHERE f.map_run_id = $runId ORDER BY f.id DESC";
            }
            $frames = $pdo->query($sql)->fetchAll(PDO::FETCH_ASSOC);
            
            $fIds = array_column($frames, 'id');
            $proposals =[];
            $persisted =[];
            ...
            if (!empty($fIds)) {
                $in = implode(',', $fIds);
                // Staged Proposals
                $stSql = "SELECT s.id as staged_id, s.frame_id, s.tag_id, s.score, s.active, s.reviewed, t.name as tag_name
                          FROM tags_2_frames_staged s JOIN tags t ON t.id = s.tag_id WHERE s.frame_id IN ($in) ORDER BY s.score DESC";
                foreach ($pdo->query($stSql)->fetchAll(PDO::FETCH_ASSOC) as $row) {
                    $proposals[$row['frame_id']][] = $row;
                }
                
                // Persisted Tags
                $pSql = "SELECT tf.from_id as tag_id, tf.to_id as frame_id, t.name as tag_name 
                         FROM tags_2_frames tf JOIN tags t ON t.id = tf.from_id WHERE tf.to_id IN ($in)";
                foreach ($pdo->query($pSql)->fetchAll(PDO::FETCH_ASSOC) as $row) {
                    $persisted[$row['frame_id']][] = $row;
                }
            }

            foreach ($frames as &$f) {
                $f['proposals'] = $proposals[$f['id']] ?? [];
                $f['persisted'] = $persisted[$f['id']] ??[];
                $f['reviewed'] = !empty($f['proposals']) ? (int)$f['proposals'][0]['reviewed'] : 0;
            }
        ...
        if ($action === 'toggle_staged') {
            ...
            $pdo->prepare("UPDATE tags_2_frames_staged SET active = ? WHERE id = ?")->execute([$active, $stagedId]);
            echo json_encode(['status'=>'success']);
            exit;
        }
        ...
        if ($action === 'set_reviewed') {
            ...
            $pdo->prepare("UPDATE tags_2_frames_staged SET reviewed = ? WHERE frame_id = ?")->execute([$reviewed, $frameId]);
            echo json_encode(['status'=>'success']);
            exit;
        }
        ...
        if ($action === 'save_manual_tags') {
            ...
            $persisted = $pdo->query("SELECT from_id FROM tags_2_frames WHERE to_id = $frameId")->fetchAll(PDO::FETCH_COLUMN);
            ...
            if (!empty($toDeleteFromPersisted)) {
                ...
                $pdo->prepare("DELETE FROM tags_2_frames WHERE to_id = $frameId AND from_id IN ($inDel)")->execute();
            }
            
            // Update Staged Tags
            $pdo->prepare("DELETE FROM tags_2_frames_staged WHERE frame_id = ?")->execute([$frameId]);
            $stmt = $pdo->prepare("INSERT INTO tags_2_frames_staged (tag_id, frame_id, score, active, reviewed) VALUES (?, ?, 1.0, 1, 1)");
            foreach ($activeTagIds as $tid) {
                // Only insert into staging if it's not already persisted
                if (!in_array($tid, $persisted)) {
                    $stmt->execute([$tid, $frameId]);
                }
            }
            
            $proposals = $pdo->query("SELECT s.id as staged_id, s.frame_id, s.tag_id, s.score, s.active, s.reviewed, t.name as tag_name FROM tags_2_frames_staged s JOIN tags t ON t.id = s.tag_id WHERE s.frame_id = $frameId ORDER BY s.score DESC")->fetchAll(PDO::FETCH_ASSOC);
            $persistedTags = $pdo->query("SELECT tf.from_id as tag_id, tf.to_id as frame_id, t.name as tag_name FROM tags_2_frames tf JOIN tags t ON t.id = tf.from_id WHERE tf.to_id = $frameId")->fetchAll(PDO::FETCH_ASSOC);

            echo json_encode(['status'=>'success', 'proposals'=>$proposals, 'persisted'=>$persistedTags]);
            exit;
        }

        if ($action === 'process_single_frame') {
            set_time_limit(120);
            $frameId = (int)$_POST['frame_id'];
            ...
            if ($skipValidation) {
                ...
                foreach ($appliedIds as $tid) {
                    ...
                    $pdo->prepare("INSERT INTO tags_2_frames_staged (tag_id, frame_id, score, active, reviewed, run_id) VALUES (?, ?, 1.0, 1, 1, ?) ON DUPLICATE KEY UPDATE score = GREATEST(score, 1.0), reviewed = 1, run_id = VALUES(run_id)")->execute([$tid, $frameId, $runId]);
                }
                $logMsg = "Directly applied " . count($appliedIds) . " tags (AI skipped).";
            } else {
                $frame = $pdo->query("SELECT filename FROM frames WHERE id = $frameId")->fetch(PDO::FETCH_ASSOC);
                if (!$frame) throw new Exception("Frame not found");
                
                $thumbAbs = $_SERVER['DOCUMENT_ROOT'] . '/' . ltrim(strtok($frame['filename'], '?'), '/');
                if (!file_exists($thumbAbs)) throw new Exception("Image not found at $thumbAbs");
                ...
                foreach ($tagMap as $tid => $name) {
                    ...
                    if ($score > 0.5) { 
                        $pdo->prepare("INSERT INTO tags_2_frames_staged (tag_id, frame_id, score, active, reviewed, run_id) VALUES (?, ?, ?, 1, 0, ?) ON DUPLICATE KEY UPDATE score = GREATEST(score, VALUES(score)), run_id = VALUES(run_id)")->execute([$tid, $frameId, $score, $runId]);
                    }
                }
                $logMsg = "Analyzed #$frameId. Applied: " . count($appliedIds) . " tags.";
            }
        ...
        if ($action === 'persist_staged') {
            $insert = $pdo->prepare("INSERT IGNORE INTO tags_2_frames (from_id, to_id) SELECT tag_id, frame_id FROM tags_2_frames_staged WHERE reviewed = 1 AND active = 1");
            $insert->execute();
            $written = $insert->rowCount();
            $pdo->prepare("DELETE FROM tags_2_frames_staged WHERE reviewed = 1")->execute();
...
<style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .mode-btn { padding: 4px 10px; border: none; background: rgba(0,0,0,0.3); color: var(--text-muted); cursor: pointer; font-family: inherit; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; transition: all 0.15s; }
    ...
    /* IFRAME MODAL */
    #iframe-modal .modal-content { width: 95vw; max-width: 1200px; height: 88vh; padding: 8px; display: flex; flex-direction: column; }
    #entity-iframe { flex: 1; border: none; border-radius: 3px; background: var(--bg); width: 100%; height: 100%; }
</style>

<div class="eh-layout">
    <div class="eh-header">
        <div class="eh-title">
            <span>🖼️</span> FRAMES TAGGER
            <div style="margin-left: 20px; display: inline-flex; border: 1px solid var(--border); border-radius: 4px; overflow: hidden; font-size: 0.7rem;">
                <button id="modeBtnMR" class="mode-btn active" onclick="setMode('map_runs')">Map Runs</button>
                <button id="modeBtnSB" class="mode-btn" onclick="setMode('storyboards')">Storyboards</button>
            </div>
        </div>
    ...
    <div class="eh-top-panel">
        ...
        <div class="mr-list-scroll" id="mrList"><div class="state-msg">Loading results...</div></div>
    </div>
...
<div id="iframe-modal" class="modal-overlay">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <button class="btn btn-sm btn-outline-secondary" onclick="closeIframeModal()">Close</button>
        </div>
...
<script>
let curPage = 1, totalPages = 1, currentRunId = null, currentFrames =[], selectedIds = new Set();
let debounceTimer;
let allTags =[];
let currentMode = 'map_runs'; // or 'storyboards'
...
function setMode(mode) {
    currentMode = mode;
    document.getElementById('modeBtnMR').classList.toggle('active', mode === 'map_runs');
    document.getElementById('modeBtnSB').classList.toggle('active', mode === 'storyboards');
    document.getElementById('mrSearch').placeholder = mode === 'map_runs' ? 'Search Map Run...' : 'Search Storyboard...';
    currentRunId = null;
    document.getElementById('gridState').style.display = 'flex';
    document.getElementById('gridState').innerHTML = `<div>&#8593; Select a ${mode === 'map_runs' ? 'Map Run' : 'Storyboard'}</div>`;
...
function loadMapRuns(page) {
    ...
    const endpoint = currentMode === 'map_runs' ? 'get_map_runs' : 'get_storyboards';
    
    fetch(`?api_action=${endpoint}&limit=6&offset=${(page-1)*6}&search=${encodeURIComponent(search)}`)
        .then(r => r.json()).then(res => {
            ...
            list.innerHTML = '';
            if(!res.data.length) { list.innerHTML = '<div class="state-msg">No results found</div>'; return; }
            
            res.data.forEach(run => {
                const el = document.createElement('div');
                el.className = `mr-item ${run.id == currentRunId ? 'active' : ''}`;
                el.onclick = () => selectRun(run.id, el);
                el.innerHTML = `<div class="mr-id">#${run.id}</div><div class="mr-note">${run.note||'No note'}</div><div class="mr-meta">${run.item_count} frames</div>`;
                list.appendChild(el);
            });
        });
}
...
function selectRun(runId, el) {
    ...
    const paramName = currentMode === 'map_runs' ? 'map_run_id' : 'storyboard_id';
    
    fetch(`?api_action=get_frames&${paramName}=${runId}`).then(r => r.json()).then(res => {
...
function renderGrid() {
    ...
    currentFrames.forEach(f => {
        ...
        card.dataset.reviewed = f.reviewed;
        
        let chipsHtml = '';
        const persistedTagIds = new Set();
        
        f.persisted.forEach(p => {
        ...
        f.proposals.forEach(p => {
            ...
            chipsHtml += `<span class="tag-chip ${isOn}" onclick="toggleChip(${p.staged_id}, ${f.id}, this)">${p.tag_name}</span>`;
        });
        ...
        card.innerHTML = `
            <div class="f-thumb-wrap">
                <img src="${f.thumbnail}" class="f-thumb" loading="lazy" onclick="openManualTag(${f.id})">
                <div class="f-view-btn" onclick="openEntityForm(${f.entity_id}, '${f.entity_type}')" title="Edit Entity"><i class="bi bi-pencil-square"></i></div>
                <div class="f-select-trigger" onclick="toggleSelect(${f.id}, this.parentElement.parentElement)"></div>
            </div>
            <div class="f-tags" id="tags_${f.id}">${chipsHtml}</div>
            <div class="f-review-bar">
                ...
                <label style="font-size:0.65rem; display:flex; gap:4px; align-items:center; cursor:pointer;">
                    <input type="checkbox" class="review-check" ${f.reviewed ? 'checked' : ''} onchange="toggleReviewed(${f.id}, this.checked)">
                </label>
                <button class="action-btn" style="padding:2px 6px; font-size:0.6rem;" onclick="openManualTag(${f.id})">TAG</button>
            </div>
        `;
...
function toggleChip(stagedId, frameId, chip) {
    ...
    fetch('?api_action=toggle_staged', {method:'POST', body:fd}).then(r=>r.json()).then(res => {
        if(res.status !== 'success') {
            ...
            const f = currentFrames.find(frm => frm.id === frameId);
            const p = f.proposals.find(prop => prop.staged_id == stagedId);
...
function toggleReviewed(frameId, checked) {
    ...
    fetch('?api_action=set_reviewed', {method:'POST', body:fd}).then(r=>r.json()).then(res => {
        if(res.status !== 'success') Toast.show('Error updating', 'error');
            ...
            const f = currentFrames.find(frm => frm.id === frameId);
            if(f) f.reviewed = checked ? 1 : 0;
            const c = document.querySelector(`.f-card[data-id="${frameId}"]`);
...
// ── MANUAL MODAL ──
let manualCurrentFrameId = null;
let manualSelectedTags = new Set();

function openManualTag(id) {
    manualCurrentFrameId = id;
    const f = currentFrames.find(frm => frm.id === id);
    document.getElementById('manualThumb').src = f.thumbnail;
    
    manualSelectedTags.clear();
    f.persisted.forEach(p => manualSelectedTags.add(p.tag_id));
    f.proposals.forEach(p => { if(p.active) manualSelectedTags.add(p.tag_id); });
...
function saveManualTags() {
    const fd = new FormData();
    fd.append('frame_id', manualCurrentFrameId);
    ...
    fetch('?api_action=save_manual_tags', {method:'POST', body:fd}).then(r=>r.json()).then(res => {
        if(res.status === 'success') {
            ...
            const f = currentFrames.find(frm => frm.id === manualCurrentFrameId);
            if(f) { 
                f.proposals = res.proposals; 
                f.persisted = res.persisted;
                f.reviewed = 1; 
            }
            renderGrid();
        }
...
function openBatchModal() {
    if(selectedIds.size === 0) return Toast.show('No frames selected');
    ...
    selectedIds.forEach(id => {
        const f = currentFrames.find(frm => frm.id === id);
        if(f) gc.innerHTML += `<div class="batch-card" id="bcard-${id}"><img src="${f.thumbnail}"></div>`;
    });
    
    document.getElementById('batchConsole').innerHTML = '';
    logBatch(`Ready to tag ${selectedIds.size} frames.`);
...
async function startBatchTagging() {
    ...
    for (const frameId of Array.from(selectedIds)) {
        const card = document.getElementById(`bcard-${frameId}`);
        ...
        try {
            ...
            const r = await fetch('?api_action=process_single_frame', {method:'POST', body:fd});
            ...
            if(res.status === 'success') {
                ...
                logBatch(`Error #${frameId}: ${res.message}`, 'error');
            }
        } catch(e) {
            if(card) card.classList.remove('processing');
            logBatch(`Network Error #${frameId}`, 'error');
        }
    }
...
function openEntityForm(entityId, entityType) {
    if (!entityId) {
        Toast.show('No linked entity available', 'error');
        return;
    }
...
function closeIframeModal() {
    ...
    document.getElementById('entity-iframe').src = '';
}
