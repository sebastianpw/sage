<?php
// public/enhancestor.php
// Enhancestor -- Frame Enhancement Interface
// ----------------------------------------------------
require_once __DIR__ . '/bootstrap.php';
require_once __DIR__ . '/env_locals.php';

// ... [API HANDLER CODE REMAINS UNCHANGED] ...
if (isset($_REQUEST['api_action'])) {
    ...
    try {
        ...
        if ($action === 'get_frames') {
            ...
            $sql = "SELECT f.id as frame_id, f.filename, s.id as sketch_id, s.name as sketch_name,
                    CASE WHEN fe.id IS NOT NULL THEN 1 ELSE 0 END as is_enhanced
                    FROM frames f 
                    JOIN sketches s ON f.entity_id = s.id 
        ...
        if ($action === 'submit_enhancements') {
            $input = json_decode(file_get_contents('php://input'), true);
            $frameIds    = $input['frame_ids'] ?? [];
            $description = trim($input['description'] ?? '');
            if (empty($frameIds)) throw new Exception("No frames selected.");
            if (empty($description)) throw new Exception("Please provide an enhancement description.");
...
<style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .eh-title { font-size: 1rem; font-weight: 700; letter-spacing: 1px; color: var(--amber); display: flex; align-items: center; gap: 8px; }
    ...
    .nav-link-btn.active { background: var(--bg); color: var(--amber); border-bottom: 2px solid var(--amber); }
    ...
    .pg-input:focus { outline: none; border-color: var(--amber); }
    ...
    .config-bar { padding: 10px 12px; display: flex; gap: 10px; }
    .prompt-input { width: 100%; padding: 8px 10px; border-radius: 4px; border: 1px solid var(--border); background: rgba(0,0,0,0.3); color: var(--text); font-family: inherit; font-size: 0.9rem; }
    ...
    .chk-label input { accent-color: var(--amber); }
    ...
    .f-card.selected { border-color: var(--amber); }
    .f-card.is-enhanced { border-color: #333; opacity: 0.4; filter: grayscale(50%); }
    .f-card.is-enhanced::before { content: "ENHANCED"; position: absolute; top: 40%; left: 0; right: 0; text-align: center; font-size: 0.7rem; font-weight: 900; color: rgba(255,255,255,0.4); transform: rotate(-15deg); pointer-events: none; z-index: 5; }
    ...
    .f-card.selected .f-label { background: var(--amber-dim); color: var(--amber); border-top-color: var(--amber); }
    ...
    .submit-btn { padding: 12px 24px; border-radius: 4px; background: var(--amber); color: #000; border: none; font-size: 0.9rem; font-weight: 700; text-transform: uppercase; cursor: pointer; font-family: inherit; transition: filter 0.15s; }
    ...
    .spinner { width: 20px; height: 20px; border: 2px solid rgba(245,158,11,0.2); border-top-color: var(--amber); border-radius: 50%; animation: spin 0.8s linear infinite; }
...
<div class="eh-layout">
    <div class="eh-header">
        ...
        <div class="eh-nav">
            <a href="enhancestor.php" class="nav-link-btn active">Enhancestor</a>
    ...
    <div class="eh-mid-panel">
        <div class="config-bar">
            <input type="text" class="prompt-input" id="enhancePrompt" placeholder="Instruction: e.g. 'remove text', 'make darker'..." value="Remove all speech bubbles, text boxes, captions and text while preserving everything exactly as is">
        </div>
    ...
    <div class="eh-footer">
        ...
        <button class="submit-btn" id="submitBtn" disabled onclick="submitEnhancements()">
            Enhance Frames
        </button>
    </div>
</div>
...
<script>
// [JS LOGIC SAME AS PREVIOUS - WITH NAV]
let curPage = 1;
let totalPages = 1;
...
function renderGrid() {
    const grid = document.getElementById('framesGrid');
    const hideEnhanced = document.getElementById('hideEnhanced').checked;
    grid.innerHTML = '';
    currentFrames.forEach(f => {
        const isEnhanced = parseInt(f.is_enhanced) === 1;
        const card = document.createElement('div');
        card.className = 'f-card';
        if (isEnhanced) card.classList.add('is-enhanced');
        if (isEnhanced && hideEnhanced) card.classList.add('hidden-in-grid');
        card.dataset.fid = f.frame_id;
        card.dataset.isEnhanced = isEnhanced ? "1" : "0";
...
function applyGridFilters() {
    ...
    cards.forEach(card => {
        const isEnhanced = card.dataset.isEnhanced === "1";
        if (isEnhanced && hideEnhanced) {
...
function submitEnhancements() {
    ...
    if (!prompt) { Toast.show('Enter instruction', 'error'); return; }
    const btn = document.getElementById('submitBtn');
    const orig = btn.textContent;
    btn.disabled = true;
    btn.textContent = '...';
    fetch('?api_action=submit_enhancements', {
        method: 'POST', headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({ frame_ids: Array.from(selectedFrameIds), description: prompt })
    }).then(r => r.json()).then(res => {
        if (res.status === 'success') {
            Toast.show(`Queued ${res.count} frames`);
            selectedFrameIds.clear();
            document.getElementById('enhancePrompt').value = 'Remove all speech bubbles, text boxes, captions and text while preserving everything exactly as is';
            if(currentRunId) {
    ...
    }).finally(() => { btn.disabled = false; btn.textContent = orig; updateSummary(); });
}
