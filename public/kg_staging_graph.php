...
// Fetch nodes
$stmtNodes = $pdo->query("SELECT id, name, node_type FROM kg_staging_nodes WHERE status='active'");
$dbNodes = $stmtNodes->fetchAll(PDO::FETCH_ASSOC);

// Fetch edges (only mapping active internal kg_staging_nodes)
$stmtEdges = $pdo->query("
...
/* ══════════════════════════════════════════════
   FILTER MODAL STYLES  (mirrors export modal)
   ══════════════════════════════════════════════ */
.kgf-modal-bg {
...
<div class="kg-layout">
    <div class="kg-topbar">
        ...
        <div style="margin-left: auto; display: flex; gap: 8px;">
            <button class="btn btn-ghost btn-sm" onclick="window.location.href='kg_staging.php'"><i class="bi bi-list"></i> Standard Tree View</button>
        </div>
    </div>
...
// ══════════════════════════════════════════════
const KGF_STORAGE_KEY    = 'kg_staging_graph_filter';
const KGF_OPEN_KEY       = 'kg_staging_graph_filter_open';
...
function kgfSaveToStorage(ids) {
    // ids: Set of DB node id strings, or null for "all"
    try {
        if (ids === null) {
...
// Derive which DB node IDs are visible from the current kgfChecked set
...
// Check if the filter is "all nodes selected" — if so treat as no filter
...
// Apply filter to graph: hide nodes not in active set
...
function kgfLoadTree() {
    ...
    fetch('kg_staging_api.php?action=fetch_tree')
        ...
        .then(res => {
            ...
            // Initialise checked set: if we have a stored active filter, match it;
            ...
            if (kgfActiveIds !== null) {
                ...
                folders.forEach(folder => {
                    ...
                    if (checkedCount === children.length) {
                        kgfChecked.add(folder.id);    // all checked → folder fully checked
                    } else {
                        kgfChecked.delete(folder.id); // partial/none → unchecked (indeterminate rendered below)
                    }
                });
            } else {
...
// ── Initialise filter from localStorage on page load ──
function kgfInitFromStorage() {
    const stored = kgfLoadFromStorage();
    // stored is a Set of DB node id strings or null
...
function openNodePanel(nodeId) {
    ...
    // Ensure content is visible if previously collapsed
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btn-open-node').addEventListener('click', () => {
        if (selectedNode) window.open('kg_staging.php?node_id=' + selectedNode, '_blank');
    });
...
function openDetailsModal(nodeId, addToHistory = true) {
    ...
    $.get('kg_staging_api.php?action=get_node&id=' + nodeId, res => {
        if (!res.ok) {
...
function createNode() {
    ...
    $.post('kg_staging_api.php', { action: 'create_node', name: name, node_type: type }, res => {
        if (res.ok) {
            hideModal('modalNode');
