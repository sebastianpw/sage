...
# Now accepts a batch map_run_id from the wrapper and stores it on the video.
# ==============================================================================
...
if [ -z "$MAP_RUN_ID" ] || [ -z "$JOB_ID" ]; then
    echo "Usage: $0 MAP_RUN_ID JOB_ID"
    exit 1
fi
...
echo "Animatic: $ANIMATIC_ID | Arrangement: $ARRANGEMENT_ID | Map Run: $MAP_RUN_ID"
...
VIDEO_ID=$(mysql $MYSQL_ARGS -N -e "
    INSERT INTO videos (map_run_id, name, description, url, thumbnail, duration, type, file_size, width, height, created_at)
    VALUES ('$MAP_RUN_ID', '$VIDEO_FILENAME', 'MultiVid Render (#$ANIMATIC_ID)', 'videos/$VIDEO_FILENAME', 'videos/thumbnails/$THUMB_FILENAME', $DURATION_S, 'video/mp4', $FILE_SIZE, $CANVAS_W, $CANVAS_H, NOW());
    SELECT LAST_INSERT_ID();
")

[ -z "$VIDEO_ID" ] || [ "$VIDEO_ID" -eq 0 ] && fail "Failed to register video"

mysql $MYSQL_ARGS -e "INSERT INTO videos_2_animatics (from_id, to_id) VALUES ($VIDEO_ID, $ANIMATIC_ID);" || fail "Failed to link video to animatic"
mysql $MYSQL_ARGS -e "UPDATE animatics SET active_map_run_id=$MAP_RUN_ID WHERE id=$ANIMATIC_ID;" || fail "Failed to update animatics active_map_run_id"
mysql $MYSQL_ARGS -e "UPDATE multivid_render_jobs SET status='completed', video_id=$VIDEO_ID, updated_at=NOW() WHERE id=$JOB_ID;" || fail "Failed to finalize job"

echo "══════════════════════════════════════════"
echo " MultiVid Render Complete | Video #$VIDEO_ID"
echo " File: $VIDEO_FILENAME"
echo " Map Run: $MAP_RUN_ID"
echo "══════════════════════════════════════════"
exit 0
