...
# ==============================================================================
# SAGE Enhancement Generation Queue Worker
# Fetches pending enhancement tasks from map_run_queue and processes them.
# Usage: ./genhance_queue.sh [limit]
# ==============================================================================
...
# -----------------------------
QUERY="
  SELECT id, map_run_id, entity_id, attempts, max_attempts, api_provider_config
  FROM map_run_queue 
...
declare -a Q_IDS Q_MAP_RUNS Q_ENTITY_IDS Q_ATTEMPTS Q_MAX_ATTEMPTS Q_CONFIGS
while IFS=$'\t' read -r q_id m_id e_id att max_att conf; do
    Q_IDS+=("$q_id")
    Q_MAP_RUNS+=("$m_id")
    Q_ENTITY_IDS+=("$e_id")
    Q_ATTEMPTS+=("$att")
...
if [ "$TOTAL_COUNT" -eq 0 ]; then
    echo "No pending enhancement tasks in queue."
    exit 0
fi

echo "========================================================"
echo "Processing $TOTAL_COUNT queued enhancement tasks..."
echo "========================================================"
...
for i in "${!Q_IDS[@]}"; do
    ...
    ENHANCEMENT_ID="${Q_ENTITY_IDS[$i]}"
    ATTEMPTS="${Q_ATTEMPTS[$i]}"
    MAX_ATTEMPTS="${Q_MAX_ATTEMPTS[$i]}"
    CONFIG_JSON="${Q_CONFIGS[$i]}"
    
    echo ""
    echo "[Task $QUEUE_ID] Starting Enhancement #$ENHANCEMENT_ID (MapRun: $MAP_RUN_ID)"
    ...
    # Fetch original enhancement row precisely safely
    RAW_DATA=$(mysql $MYSQL_ARGS -N -e "
      ...
      WHERE id=$ENHANCEMENT_ID LIMIT 1;
    ")
    
    if [ -z "$RAW_DATA" ]; then
        mysql $MYSQL_ARGS -e "UPDATE map_run_queue SET status = 'failed', error_msg = 'Enhancement record not found', completed_at = NOW() WHERE id = $QUEUE_ID;"
        echo "✗ Task Failed: Enhancement record not found."
        continue
    fi

    IFS=$'\x1f' read -r ENTITY_PROMPT ENTITY_SPECIFIC_NEGATIVE ENTITY_SEED PROMPT_TYPE ORIG_ENTITY_ID SOURCE_FRAME_ID <<< "$RAW_DATA"
    ...
    ENTITY_TYPE="$PROMPT_TYPE"
    ENTITY_ID="$ORIG_ENTITY_ID"
    FORCED_IMG2IMG_ID="$SOURCE_FRAME_ID"
    ...
    if [ -z "$ORIG_ENTITY_DESC" ]; then ORIG_ENTITY_DESC="$BASE_PROMPT"; fi
        ...
        if [ -n "$ENHANCEMENT_ID" ] && [ "$ENHANCEMENT_ID" -gt 0 ]; then
            ...
            if [ ${#enhancement_frame_ids[@]} -gt 0 ]; then
                ...
                for extra_frame_id in "${enhancement_frame_ids[@]}"; do
                    ...
                    if [ -n "$extra_frame_filename" ]; then
    ...
    if [ -n "$ERROR_MSG" ]; then
        ...
        echo "✓ Frame enhancements generated successfully."
