...
INPUT_PATH="frames_collab/frame%07d.jpg"
#INPUT_PATH="frames_pollinations/frame%07d.jpg"

TIMESTAMP=$(date +%Y%m%d_%H%M%S)
OUTPUT="videos/video_${TIMESTAMP}_360p.mp4"
...
echo "🎬 Creating 360p video"
echo "Framerate: $FRAMERATE fps"
echo "GOP size: $GOP"
echo "Boxblur: $BOXBLUR"
[ -n "$AUDIO_FILE" ] && echo "Using audio file: $AUDIO_FILE"

#INPUT_PATH="frames_pollinations/frame%07d.jpg"

FILTER_COMPLEX="
[0:v]scale=640:360,format=yuv420p,boxblur=${BOXBLUR}[bg];
[0:v]scale=360:360,format=yuv420p[fg];
[bg][fg]overlay=(W-w)/2:(H-h)/2:format=auto[tmp];
[tmp]scale=640:360,setsar=1,format=yuv420p[out]
"

COMMON_OPTS="-vframes $NUM_IMAGES \
-c:v libx264 -crf 20 -preset slow -pix_fmt yuv420p -g $GOP \
-profile:v baseline -level 3.1 -s 640x360 -movflags +faststart"
...
echo "✅ 360p video created: $OUTPUT"
