#!/bin/bash
# save as: stop_genvideos.sh

echo "Attempting to stop genvideos_fromdb.sh..."

# Method 1: Kill by specific script name
if pkill -f "genvideos_fromdb.sh"; then
    echo "✅ genvideos_fromdb.sh process terminated"
else
    echo "⚠️  No genvideos_fromdb.sh process found by name"
fi

# Method 2: If you have the scheduler PID file
if [ -f "/path/to/scheduler.pid" ]; then
...
REMAINING=$(ps aux | grep -E "genvideos_fromdb\.sh" | grep -v grep)
if [ ! -z "$REMAINING" ]; then
    echo "Force killing remaining processes..."
    pkill -9 -f "genvideos_fromdb.sh"
fi
